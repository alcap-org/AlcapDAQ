...
//    diag_print(0, "Unexpected status 0x%08x on %s after reset, disabling\n", 
    printf("Unexpected status 0x%08x on %s after reset, disabling\n", 
      csr, sis->odb_name);
    sis->enabled = false;
    return FE_ERR_HW;
  }
...
//    diag_print(0, "Unexpected status 0x%08x on %s after setup, disabling\n", 
    printf("Unexpected status 0x%08x on %s after setup, disabling\n", 
      csr, sis->odb_name);
    sis->enabled = false;
    return FE_ERR_HW;
  }

  printf("Enabled %s successfully\n", sis->odb_name);
...
INT sis3600_read1(struct sis3600 *sis, char *pevent)
{
  ...
  int total_size = active_size + final_size;
  if ( total_size > 500000 ) {
    printf("ERROR! Event size is too big (%i). The event will be rejected\n");
