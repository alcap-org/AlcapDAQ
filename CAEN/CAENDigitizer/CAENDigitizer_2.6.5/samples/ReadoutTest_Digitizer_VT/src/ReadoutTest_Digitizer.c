...
int main(int argc, char* argv[])
{
    ...
    for(b=0; b<MAXNB; b++){
        ...
        //ret = CAEN_DGTZ_OpenDigitizer(CAEN_DGTZ_OpticalLink,0,b,0x00000000,&handle[b]);
      // ret = CAEN_DGTZ_OpenDigitizer(CAEN_DGTZ_OpticalLink,0,b,0x32100000,&handle[b]);
        ret = CAEN_DGTZ_OpenDigitizer(CAEN_DGTZ_OpticalLink,0,2,0x32100000,&handle[b]);
        ...
        ret = CAEN_DGTZ_SetAcquisitionMode(handle[b],CAEN_DGTZ_S_IN_CONTROLLED);          /* Set the acquisition mode */
        if(ret != CAEN_DGTZ_Success) {
