...
int main(int argc, char* argv[])
{
	...
	ret = CAEN_DGTZ_SetChannelEnableMask(handle[b],0xFF);                              /* Enable channel 0 */
	ret = CAEN_DGTZ_SetChannelTriggerThreshold(handle[b],0,512);                  /* Set selfTrigger threshold */
	
	// trigger threshold
	ret = CAEN_DGTZ_WriteRegister(handle[b], 0x1080, 15000); 

	// trigger nsamples over threshold
	ret = CAEN_DGTZ_WriteRegister(handle[b], 0x1084, 2); 

	ret = CAEN_DGTZ_SetChannelPulsePolarity(handle[b], 0, CAEN_DGTZ_PulsePolarityNegative);	
	ret = CAEN_DGTZ_SetChannelDCOffset(handle[b], 0, 0);

	//ret = CAEN_DGTZ_SetChannelSelfTrigger(handle[b],CAEN_DGTZ_TRGMODE_ACQ_ONLY,1);  /* Set trigger on channel 0 to be ACQ_ONLY */
	ret = CAEN_DGTZ_SetChannelSelfTrigger(handle[b],CAEN_DGTZ_TRGMODE_ACQ_AND_EXTOUT,0x1);  /* Set trigger on channel 0 to be ACQ_AND_EXTOUT */
	//ret = CAEN_DGTZ_SetSWTriggerMode(handle[b],CAEN_DGTZ_TRGMODE_ACQ_ONLY);         /* Set the behaviour when a SW tirgger arrives */
	ret = CAEN_DGTZ_SetSWTriggerMode(handle[b],CAEN_DGTZ_TRGMODE_ACQ_AND_EXTOUT);         /* Set the behaviour when a SW tirgger arrives */
	ret = CAEN_DGTZ_SetMaxNumEventsBLT(handle[b],1024);                                /* Set the max number of events to transfer in a sigle readout */
        ret = CAEN_DGTZ_SetAcquisitionMode(handle[b],CAEN_DGTZ_SW_CONTROLLED);          /* Set the acquisition mode */

	ret = CAEN_DGTZ_WriteRegister(handle[b], 0x800C, 0xA); /* make 1024 buffers */

	ret = CAEN_DGTZ_WriteRegister(handle[b], 0x8100, 0x8); /* count all triggers */

	ret = CAEN_DGTZ_WriteRegister(handle[b], 0x8114, 32); /* post-trigger samples */

	// front panel trigger out enable mask
	ret = CAEN_DGTZ_WriteRegister(handle[b], 0x8110, 0xC00000FF); 

	// DAC monitor mode: voltage level monitor
	ret = CAEN_DGTZ_WriteRegister(handle[b], 0x8144, 0x4); 

	// Front panel io control
	ret = CAEN_DGTZ_WriteRegister(handle[b], 0x811C, 0x0); 
	
	CAEN_DGTZ_SetOutputSignalMode(handle[b], CAEN_DGTZ_TRIGGER);
	...
	while(1) {
        for(b=0; b<MAXNB; b++) {
		    ...
		    printf("SW trigger status: %i\n",ret);

		    //ret = CAEN_DGTZ_WriteRegister(handle[b], 0x8108, 0xFFFF); /* Send a SW trigger */
		    // printf("SW trigger status: %i\n",ret);

		    ret = CAEN_DGTZ_WriteRegister(handle[b], 0x8138, 1); /* SET DAC */
		    printf("Write DAC status: %i\n",ret);


		    uint32_t xxx;
		    ret = CAEN_DGTZ_ReadRegister(handle[b], 0x812C, &xxx);
		    printf("Event Stored: 0x%08x ret = %i\n",xxx, ret);

		    ret = CAEN_DGTZ_ReadRegister(handle[b], 0x814C, &xxx);
		    printf("Event size: 0x%08x ret = %i\n",xxx, ret);

		    ret = CAEN_DGTZ_ReadRegister(handle[b], 0x810C, &xxx);
		    printf("Trigger Source enable mask: 0x%08x ret = %i\n",xxx, ret);
		    
		    ret = CAEN_DGTZ_ReadRegister(handle[b], 0x1088, &xxx);
		    printf("Channel 0 status: 0x%08x ret = %i\n",xxx, ret);

		    ret = CAEN_DGTZ_ReadRegister(handle[b], 0x1188, &xxx);
		    printf("Channel 1 status: 0x%08x ret = %i\n",xxx, ret);

		    ret = CAEN_DGTZ_ReadRegister(handle[b], 0x1094, &xxx);
		    printf("Channel 0 buffer occupancy: 0x%08x ret = %i\n",xxx, ret);

		    ret = CAEN_DGTZ_ReadRegister(handle[b], 0x8100, &xxx);
		    printf("Acquision Control: 0x%08x ret = %i\n",xxx, ret);

		    ret = CAEN_DGTZ_ReadRegister(handle[b], 0x8104, &xxx);
		    printf("Acquision Status: 0x%08x ret = %i\n",xxx, ret);

		    ret = CAEN_DGTZ_ReadRegister(handle[b], 0x8114, &xxx);
		    printf("Post trigger samples: 0x%08x ret = %i\n",xxx, ret);
		    ...
		    ret = CAEN_DGTZ_WriteRegister(handle[b], 0x8010, 1024); /* Free buffers */
		    printf("Free buffers status: %i\n",ret);

		    ret = CAEN_DGTZ_WriteRegister(handle[b], 0x8138, 1024); /* SET DAC */
		    printf("Write DAC status: %i\n",ret);
		    ...
		    // front panel trigger out enable mask
		    ret = CAEN_DGTZ_WriteRegister(handle[b], 0x8110, 0x0); 
