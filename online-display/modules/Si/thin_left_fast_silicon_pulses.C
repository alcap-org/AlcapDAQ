void thin_left_fast_silicon_pulses()
{
  /*****************************************************************/
  // Prepare the canvas
  ...
  /*****************************************************************/
	...
	for (int j = 0; j < 4; ++j) // loop through the channels
	{
		...
		while (final_pulse != true) {
			sprintf(pulseHistTitle,"hSiL1_%dFastRaw_Event%d_Island%d_Pulse%d;1",j+1, event_number, island_number, pulse_number);
			left_fast_pulse[j][pulse_number-1] = (TH1 *)gDirectory->Get(pulseHistTitle);
			
			if (left_fast_pulse[j][pulse_number-1]) {
				left_fast_pulse[j][pulse_number-1]->SetLineColor(kMagenta);
				left_fast_pulse[j][pulse_number-1]->Draw("SAME");
				pulse_number++;
			}
			else
				final_pulse = true;
