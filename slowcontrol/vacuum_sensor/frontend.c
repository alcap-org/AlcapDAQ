...
INT frontend_init()
{
  ...
  if (vacuum == -1) {
    cm_msg(MERROR, "vacuum_gauge_init",
	   "Error opening vacuum gauge (errno %d)", errno);
    return FE_ERR_HW;
  }
  ret = tcgetattr(vacuum, &vacuum_config);
  if (ret == -1) {
    cm_msg(MERROR, "vacuum_gauge_init",
	   "Error getting vacuum gauge serial port attributes (errno %d)", errno);
    return FE_ERR_HW;
  }
  ...
  if (ret == -1) {
    cm_msg(MERROR, "vacuum_gauge_init",
	   "Error setting vacuum gauge serial port attributes (errno %d)", errno);
    return FE_ERR_HW;
  }
...
INT vacuum_gauge_read(char *pevent, INT off) {
  ...
  if (ret == -1) {
    cm_msg(MERROR, "read_vacuum_gauge",
	   "Cannot write to vacuum gauge to request reading (errno %d)", errno);
    return FE_ERR_HW;
  }
  ...
  if (ret == -1) {
    ...
    cm_msg(MERROR, "read_vacuum_gauge",
	   "Received to few bytes (expected 15, received %d)", ret);
    return FE_ERR_HW;
  }
