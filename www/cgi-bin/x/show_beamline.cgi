...
if( $type eq "run" ) {
    ...
    $sth = $dbh->prepare("select min(starttime),max(stoptime) from Runlog ".
    ...
    $sth = $dbh->prepare("select min(run),max(run) from Runlog ".
    ...
    $sth->finish;
    $sth = $dbh->prepare("select min(run) from Runlog where stoptime>? and starttime<? and run>9000");
    if( ! $sth->execute($tmin,$tmax) ) { printError("Database error : ".$DBI::errstr); }
    ...
    ( $tmin ) = $sth->fetchrow_array();
    $sth->finish;
    $sth = $dbh->prepare("select min(run) from Runlog where stoptime>? and starttime<?");
    if( ! $sth->execute($tmin,$tmax) ) { printError("Database error : ".$DBI::errstr); }
    ( $rmin ) = $sth->fetchrow_array();
    $rmax = $rmax0;
    $sth->finish;
    ...
    $rmax = $rmax0;
    $sth = $dbh->prepare("select run from Runlog where run>9000 order by run desc limit 20");
    $sth->execute();
    $rmin = 10000000;
    while( my @data = $sth->fetchrow_array() ) {
    ...
    $sth = $dbh->prepare("select min(starttime) from Runlog where starttime is not null and run>? ");
    ...
    $rmax = $rmax0;
    $sth = $dbh->prepare("select run from Runlog where run>9000 order by run desc limit 100");
    $sth->execute();
    $rmin = 10000000;
    while( my @data = $sth->fetchrow_array() ) {
        if( $data[0]<$rmin ) { $rmin=$data[0]; }
...
if( defined($type) ) {
    ...
    if( system("convert -quality 10 -size 1000x700 $rootdir/$tmpepsfile -resize 1000x700 $outpngfile") ) {
...
if( defined($type) ) {
    ...
    print img({-src=>"/mulan/trend/$outname.png"});
