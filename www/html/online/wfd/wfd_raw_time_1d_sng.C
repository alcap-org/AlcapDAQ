...
void wfd_raw_time_1d_sng(const int run_nr = 0,
			 const int crate = 1,
			 const int slot1 = 2,
			 const int slot2 = 2) 
{
  ...
  const char* cnames[6] = { "B0", "R1", "G1", "R0", "B1", "G0" };

  sprintf( name, "WFDRawTime%s",cnames[crate-1]);  
  ...
  for (slot=slot1; slot<=slot2; slot++) {
    ...
    for (chan=0; chan<4; chan++) {
      ...
      sprintf( name, "WFDRawTime%s_slot_%02d_ch_%1d",cnames[crate-1],slot,chan); 
      int bin = h2->GetYaxis()->FindBin( (slot-1)*4+chan );
      ...
      //h1->GetXaxis()->SetRangeUser(17000.0,24000.0);
      h1->SetLineColor(kBlue);
      //h1->SetFillColor(kYellow);
      h1->SetLineWidth(2);
      h1->GetXaxis()->SetRangeUser(-0.5,12000.0);
  ...
  c1->SaveAs("../png/wfd_raw_time_1d_sng.png");
