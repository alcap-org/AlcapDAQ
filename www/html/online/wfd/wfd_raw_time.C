...
void wfd_raw_time(const int run_nr = 0) 
{
  int i;
  ...
  for (i=0; i<n_hists; i++) {
    ...
    h2[i]->SetXTitle("time counter");
    h2[i]->SetYTitle("(slot-1)*4 + chan.");
    //h2[i]->GetXaxis()->SetRangeUser(-0.5,15000.0);
    h2[i]->GetXaxis()->SetRangeUser(-0.5,24000.0);
  ...
  c1->SaveAs("../png/wfd_raw_time.png");
