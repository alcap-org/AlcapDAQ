...
void wfd_raw_fill_1d_sng(const int run_nr = 0,
			 const int crate = 1,
			 const int slot1 = 2,
			 const int slot2 = 2) 
{
  ...
  sprintf( name, "WFDRawFill%s",cnames[crate-1]);  
  ...
  for (slot=slot1; slot<=slot2; slot++) {
    ...
    for (chan=0; chan<4; chan++) {
      ...
      sprintf( name, "WFDRawFill%s_slot_%02d_ch_%1d",cnames[crate-1],slot,chan); 
      ...
      //h1->GetXaxis()->SetRangeUser(-0.5,100.0);
      h1->GetXaxis()->SetRangeUser(49000.0,51000.0);
  ...
  c1->SaveAs("../png/wfd_raw_fill_1d_sng.png");
