...
void wfd_raw_coinc_Dt(const int run_nr = 0) 
{
  int i;

  const int n_hists = 6;
  const char *h_names[n_hists] = { "WFDRawCoincDtR0",
				   "WFDRawCoincDtR1",
				   "WFDRawCoincDtG0",
				   "WFDRawCoincDtG1",
				   "WFDRawCoincDtB0",
  ...
  for (i=0; i<n_hists; i++) {
    ...
    h2[i]->Draw("ColZ");
    h2[i]->SetXTitle("time between islands");
    h2[i]->SetYTitle("slot number");
  ...
  c1->SaveAs("../png/wfd_raw_coinc_Dt.png");
