...
void wfd_raw_area_1d(const int run_nr = 0,
		     const int crate = 1,
		     const int slot1 = 2,
		     const int slot2 = 2) 
{
  ...
  sprintf( name, "WFDRawArea%s",cnames[crate-1]);  
  ...
  for (slot=slot1; slot<=slot2; slot++) {
    ...
    for (chan=0; chan<4; chan++) {
      ...
      sprintf( name, "WFDRawArea%s_slot_%02d_ch_%1d",cnames[crate-1],slot,chan); 
      ...
      h1->GetXaxis()->SetRangeUser(0.0,5000.0);
      h1->SetLineColor(kBlue);
      h1->SetFillColor(kYellow);
      h1->SetLineWidth(2);

      h1->SetXTitle("waveform area");
      h1->SetYTitle("counts");
  ...
  c1->SaveAs("../png/wfd_raw_area_1d.png");
