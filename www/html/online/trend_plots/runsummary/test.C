...
void test()
{
  ...
  TH1D * htarget[] = {hq, hag, hak};
    htarget[0]->SetFillColor(20);
    htarget[1]->SetFillColor(30);
    htarget[2]->SetFillColor(40);
  ...
  //hquery * hdata;
  hquery * hdata [nrows];
  //hdata = new hquery [nrows]; 
  // hquery * hdata = new hquery[nrows];
	  ...
	  hfound=hdata[i].hinge.find(hbin[k]);*/
	   ...
	   hfound=hdata[i]->GetHinge().find(hbin[k]);
        ...
        if (tfound!=string::npos &&  hfound!=string::npos) {
          ...
          if (k==0)
            //htarget[j]->SetBinContent(j+2, hdata[i].hsize())
  ...
  /*TCanvas c1("c1","hinge",10, 10, 700, 900);
  HingeOrient->Draw("nostack");*/
