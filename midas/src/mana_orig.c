/********************************************************************\
  ...
  *** muCap patches ***
  - When running as an online analyzer, in order to be prepared for the next 
    run, skip over events that are queued but not yet processed at the end 
    of the run.
  - Added support for a secondary ROOT output file with the '-O' option, so 
    that it is possible to create both a MIDAS file and a histogram file.  This
    feature is used by the skimmer.
  - Changed "int" to PTYPE a few places to be 64-bit clean.
  - Increased threshold for warning message from 50% to 80% of event 
    buffer size.
  - Turned off buffer cache.
  - Commented out "assert(run_number > 0)"
  - Zero-fill event buffer as it is read in (needed to avoid errors in computing
    checksum because of holes in the stacked bank data structure)
  - Only start ROOT server if running as an online analyzer.

  $Log: mana.c,v $
  Revision 1.2  2005/06/21 00:20:22  mucap
  (Fred)
    - When running as an online analyzer, in order to be prepared for the next
      run, skip over events that are queued but not yet processed at the end
      of the run.
