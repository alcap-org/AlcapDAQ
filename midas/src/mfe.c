/********************************************************************\
  ...
  ** muCap patches **
  - changed format of frontend name to avoid adding extra 0s ("Crate 3"
    rather than "Crate 03")
  ...
  $Log: mfe.c,v $
  Revision 1.3  2005/10/31 14:15:17  mucap
  (Fred)
  Changed frontend name to avoid extra 0s ("Crate 4" rather than "Crate 04")

  Revision 1.2  2005/06/21 00:53:51  mucap
  (Fred)
    - added frontend_early_init() to allow the frontend to modify its name
      and equipment structure.
    - added extra transition function pre_begin_of_run().
...
INT register_equipment(void)
{
   ...
   for (index = 0; equipment[index].name[0]; index++) {
      ...
      if (eq_info->eq_type & EQ_EB) {
         ...
         sprintf(equipment[index].name+strlen(equipment[index].name), "%d", frontend_index);

         /* modify event buffer name to <name>xx where xx is the frontend index*/
         sprintf(eq_info->buffer+strlen(eq_info->buffer), "%d", frontend_index);
      }
...
#endif
{
  ...
  frontend_early_init();
   ...
   if (frontend_index >= 0)
      sprintf(full_frontend_name+strlen(full_frontend_name), "%d", frontend_index);
