...
TF1* FitGeSpectrum(TH1* h) {
  ...
  f->SetParameters(700., 0., 5.2e3, 345., 1.7, 600., 350.);
...
TLatex* BuildDatasetLabel() {
  TLatex* txt = new TLatex(360., 30.0e3, "Al 100#mum");
  txt->SetTextSize(0.06);
  return txt;
}

TLatex* BuildChi2Label() {
  TLatex* txt = new TLatex(336., 20e3, "#splitline{#chi^{2}/ndf}{=178/101}");
...
TLatex* BuildNStopLabel() {
  TLatex* txt =
  new TLatex(336., 30e3, "#splitline{134(1)#times10^{6}}{#mu-stops}");
  txt->SetTextSize(0.07);
  txt->SetTextAlign(11);
  return txt;
}
...
vector<TLatex*> BuildPeakLabels() {
  vector<TLatex*> labs;
  labs.push_back(new TLatex(270., 20.0e3, "#muPb 6-5"));
  labs.push_back(new TLatex(270., 12.0e3, "#muPb#rightarrowTl"));
  labs.push_back(new TLatex(329., 28.0e3, "#muAl 2p-1s"));
  labs.push_back(new TLatex(405., 11.7e3, "#muPb 5g_{9/2}-4f_{7/2}"));
  labs.push_back(new TLatex(475., 10.5e3, "#muPb 5g_{7/2}-4f_{5/2}"));
  labs.push_back(new TLatex(520,  11.1e3, "511 keV"));
...
TGraph* BuildPeakMarkers() {
  TGraph* g = new TGraph();
  int i = -1;
  g->SetPoint(++i, 232.,  20.6e3);
  g->SetPoint(++i, 266.,  11.0e3);
  g->SetPoint(++i, 347.,  33.4e3);
  g->SetPoint(++i, 429.,  11.8e3);
  g->SetPoint(++i, 439,   10.5e3);
  g->SetPoint(++i, 511.,  10.5e3);
  g->SetPoint(++i, 937.8, 4.8e3);
  g->SetPoint(++i, 971.7, 3.5e3);
...
void SetOutsetHistStyle(TH1* h) {
  ...
  h->GetYaxis()->SetRangeUser(0., 35e3);
  h->SetStats(false);
...
void al100_norm() {
  TFile* f = new TFile("data/Al100/nmu_10uspp/nmu.pp10us.root");
  ...
  SetInsetHistStyle(h);
  SetFitFcnStyle(ffit);
  TPad*    p = new TPad("p", "p", 0.40, 0.35, 0.85, 0.85);
  p->SetTicks();
  p->SetMargin(0.20, 0.011, 0.10, 0.05);
  p->Draw();
  p->cd();
  h->Draw();
  ffit->Draw("L SAME");
  ...
  c->SaveAs("img/cppaper/al100_norm.png");
}
