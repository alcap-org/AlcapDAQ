...
void PidEfficiency(const char *filename = "unfolding.pdt.localhisto.2m.root", const char *treeName="oTree", Int_t sigma=1) {
	...
	for(Long64_t i=0; i < tree->GetEntries(); i++) {
		...
		if(channel->Contains("SiL1") ) {
			if(SiL_cutg_p->IsInside(0.7071 * (TMath::Log10(e1+e2+e3) - TMath::Log10(e1) ), 0.7071 * (TMath::Log10(e1+e2+e3) + TMath::Log10(e1) ) ) ) {
				if(pid->Contains("p") ) eff[0][0]++;
				if(pid->Contains("d") ) eff[1][0]++;
				if(pid->Contains("t") ) eff[2][0]++;
				Ameas[0][0]++;
			}
			if(SiL_cutg_d->IsInside(0.7071 * (TMath::Log10(e1+e2+e3) - TMath::Log10(e1) ), 0.7071 * (TMath::Log10(e1+e2+e3) + TMath::Log10(e1) ) ) ) {
				if(pid->Contains("p") ) eff[0][1]++;
				if(pid->Contains("d") ) eff[1][1]++;
				if(pid->Contains("t") ) eff[2][1]++;
				Ameas[1][0]++;
			}
			if(SiL_cutg_t->IsInside(0.7071 * (TMath::Log10(e1+e2+e3) - TMath::Log10(e1) ), 0.7071 * (TMath::Log10(e1+e2+e3) + TMath::Log10(e1) ) ) ) {
