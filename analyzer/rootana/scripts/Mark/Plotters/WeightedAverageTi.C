TH1D * Plot(TH1D *hStats, const char *particle = "proton", Bool_t drawFit = kFALSE) {
	//statistical errors
	...
	for(int i = 0; i <= 40; ++i) { //lower than the 9th bin, the uncertainty are great due to many reasons
...
//	if(strcmp(particle, "proton") == 0) {
//		hWeightedAverage->Draw("E");
//	} else {
//		hWeightedAverage->Draw("E SAME");
//	}
	if(strcmp(particle, "proton") == 0) {
...
//		hWeightedAverage->GetXaxis()->SetRangeUser(3, 20);
...
//	legend->Draw("SAME");
	...
	if(drawFit) {
		if(strcmp(particle, "proton") == 0) {
			...
			std::cout << "3 - 17MeV: " << hWeightedAverage->IntegralAndError(hWeightedAverage->FindBin(3), hWeightedAverage->FindBin(16.), syst_error ) << " ± " << stats_error << " ± " << (syst_error-stats_error )<< std::endl;
	...
	//		std::cout << sigma_integral << std::endl;
			...
			std::cout << "5 - 17MeV: " << hWeightedAverage->IntegralAndError(hWeightedAverage->FindBin(5), hWeightedAverage->FindBin(16.), syst_error ) << " ± " << stats_error << " ± " << (syst_error-stats_error )<<  std::endl;
			std::cout << "Fit 5 - 40MeV: " << fit->Integral(5, 40) << std::endl;
		} else if(strcmp(particle, "triton") == 0) {
			Double_t stats_error, syst_error;
			hStats->IntegralAndError(hStats->FindBin(5), hStats->FindBin(16), stats_error);
			std::cout << "6 - 17MeV: " << hWeightedAverage->IntegralAndError(hWeightedAverage->FindBin(5), hWeightedAverage->FindBin(16.), syst_error ) << " ±"  << stats_error << " ± " << (syst_error-stats_error )<<  std::endl;
			std::cout << "Fit 6 - 40MeV: " << fit->Integral(6, 40) << std::endl;
		} else {
			Double_t stats_error, syst_error;
			hStats->IntegralAndError(hStats->FindBin(15), hStats->FindBin(19), stats_error);
			std::cout << "15 - 20MeV: " << hWeightedAverage->IntegralAndError(hWeightedAverage->FindBin(15), hWeightedAverage->FindBin(19.), syst_error ) << " ± " << stats_error << " ± " << (syst_error-stats_error )<<  std::endl;
...
for(int i=0; i<9; ++i) {
	...
	legend->Draw("SAME");
	c->Draw();
	c->SaveAs(Form("%s/AlCapData_Ti_weighted_average.pdf", getenv("R15b_OUT") ) );
}
//void WeightedAverageTi() {
//
//	TCanvas *c = new TCanvas("c", "c");
