...
//	TFile *fJohn = new TFile("en.p.t400ns.dt200ns.rebin5.pp10us.elo2e3keV.ehi12e3keV.bay5.pidcorr.root", "READ");
	TFile *fAl50 = new TFile(Form("%s/unfolded.al50.root", getenv("R15b_OUT") ), "READ");
	TFile *fAl100 = new TFile(Form("%s/unfolded.al100.root", getenv("R15b_OUT") ), "READ");

//	TH1D *h_John_r = (TH1D *)fJohn->Get("hr_e_reco_bay");
//	TH1D *h_John_l = (TH1D *)fJohn->Get("hl_e_reco_bay");

	TH1D *h_Al50_r = (TH1D *)fAl50->Get("hproton_SiR"); h_Al50_r->Rebin(2);
	TH1D *h_Al50_l = (TH1D *)fAl50->Get("hproton_SiL");

	TH1D *h_Al100_r = (TH1D *)fAl100->Get("hproton_SiR"); h_Al100_r->Rebin(2);
	TH1D *h_Al100_l = (TH1D *)fAl100->Get("hproton_SIL");
	for(int i=17; i < 21; ++i) {
	...
	h_Al100_r->Draw("E1");
	h_Al100_r->SetLineColor(kRed);
	h_Al100_r->SetTitle(";E[keV];Protons per captured #mu^{-} / 500keV");
	gTProtons->Draw("LSAME");
	h_Al50_r->Draw("E1 SAME");
	h_Al50_r->SetLineColor(kBlack);
	...
	legend->AddEntry(gTProtons, "TWIST Al75#mum","l");
	legend->AddEntry(h_Al50_r, "Al50#mum","pe");
	legend->AddEntry(h_Al100_r, "Al100#mum","pe");
	legend->Draw();
	const char *FigsDir = getenv("R15b_OUT");
	c->SaveAs(Form("%s/AlCapData_TWIST_Al100_Al50_Comparison_Log.pdf", FigsDir) );
}
