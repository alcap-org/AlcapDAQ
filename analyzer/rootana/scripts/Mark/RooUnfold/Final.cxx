...
void Process(RooUnfoldResponse *response, TH1D *hMeas, const char *arm = "SiL", std::string target = "al50", const char * particle = "proton", bool normalise = kFALSE) {
	...
	Double_t error;
	Double_t integral = hReco->IntegralAndError(hReco->GetXaxis()->FindBin(4), hReco->GetXaxis()->FindBin(7.5), error, "");
	...
	std::cout << arm << " " << particle << " 3500-10000keV: " << integral << " ± " << error << std::endl;
}
void Unfold(std::string target = "al50", std::string particle="proton", bool normalise = kFALSE)
{
	...
	//TFile *fOutputFile = new TFile(Form("%s/unfolded.%s.root", ".", target.c_str() ), "UPDATE");
