...
void SiTCalib() {
  ...
  double pulser_voltages[n_pulser_runs] = {0.1, 0.05, 0.15, 0.20};
  ...
  for (int i_si_channel = 0; i_si_channel < n_si_channels; ++i_si_channel) {
    canvasname.str("");
    canvasname << "c_SiT-" << i_si_channel+1 << "-S";
  ...
  for (int i_calib_point = 0; i_calib_point < n_calib_points; ++i_calib_point) {
    ...
    for (int i_si_channel = 0; i_si_channel < n_si_channels; ++i_si_channel) {
      ...
      histname.str("");
      histname << "PlotTAP_Amplitude_wTimeCuts/hSiT-" << i_si_channel+1 << "-S#MaxBinAPGenerator#{no_time_shift=true}_Amplitude_wTimeCuts";
	...
	std::cout << "SiT-" << i_si_channel+1 << "-S (" << calib_energies[i_calib_point] << " +/- " << calib_energy_errors[i_calib_point] << " keV): " << calib_adcs[i_si_channel][i_calib_point] << " +/- " << calib_adc_errors[i_si_channel][i_calib_point] << " ADC (Esimate was " << estimate << ", NPeaks = " << spec.GetNPeaks() << ")" << std::endl;
  ...
  for (int i_si_channel = 0; i_si_channel < n_si_channels; ++i_si_channel) {
    ...
    if ( (Int_t) result == 0) {
      ...
      std::cout << "SiT-" << i_si_channel+1 << "-S: E [keV] = " << calib_gain[i_si_channel] << " (+/- " << calib_gain_error[i_si_channel] << ") * A [ADC] + " << calib_offset[i_si_channel] << " (+/- " << calib_offset_error[i_si_channel] << ")\t(chi^2/ndf = " << result->Chi2() << " / " << result->Ndf() << " = " << result->Chi2() / result->Ndf() << ")" << std::endl;
      ...
      std::cout << "Calibration curve fit failed for SiT-" << i_si_channel+1 << "-S" << std::endl;
    }
    //    c1->SaveAs("/gpfs/home/edmonds_a/plots/2016-11-09/SiT-3-S_EnergyCalibrationCurve.pdf");
  }
  //  c_gr->SaveAs("/gpfs/home/edmonds_a/plots/2016-11-07/SiT-3-S_EnergyCalibrationCurve_Test.pdf");
  ...
  calib_script.open("scripts/r15b_insert_calib_sit.sh", std::ofstream::out);
  calib_script << "#! /bin/bash" << std::endl;
  calib_script << std::endl;
  calib_script << "Channels=( SiT-{1.." << n_si_channels << "}-S )" << std::endl;
  calib_script << "Gains=(";    
  for (int i_si_channel = 0; i_si_channel < n_si_channels; ++i_si_channel) {
  ...
  for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
    ...
    TFile* file = new TFile(filename.str().c_str(), "READ");
    TH1F* hist = (TH1F*) file->Get("PlotTAP_Amplitude/hSiT-3-S#MaxBinAPGenerator#any_Amplitude");
