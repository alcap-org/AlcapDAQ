...
void SiL3Calib() {
  ...
  double pulser_voltages[n_pulser_runs] = {0.10, 0.05, 0.15, 0.20};
  ...
  for (int i_calib_point = 0; i_calib_point < n_calib_points; ++i_calib_point) {
    ...
    if ( (Int_t) result == 0) {
      ...
      std::cout << "SiL3-S (" << calib_energies[i_calib_point] << " +/- " << calib_energy_errors[i_calib_point] << " keV): " << calib_adcs[i_calib_point] << " +/- " << calib_adc_errors[i_calib_point] << " ADC (Esimate was " << estimate << ", NPeaks = " << spec.GetNPeaks() << ")" << std::endl;
  ...
  if ( (Int_t) result == 0) {
    ...
    std::cout << "SiL3-S: E [keV] = " << calib_gain << " (+/- " << calib_gain_error << ") * A [ADC] + " << calib_offset << " (+/- " << calib_offset_error << ")\t(chi^2/ndf = " << result->Chi2() << " / " << result->Ndf() << " = " << result->Chi2() / result->Ndf() << ")" << std::endl;
    ...
    std::cout << "Calibration curve fit failed for SiL3-S" << std::endl;
  }
  //    c1->SaveAs("/gpfs/home/edmonds_a/plots/2016-11-09/SiL3-3-S_EnergyCalibrationCurve.pdf");

  //  c_gr->SaveAs("/gpfs/home/edmonds_a/plots/2016-11-07/SiL3-3-S_EnergyCalibrationCurve_Test.pdf");
  ...
  calib_script << "Channel=SiL3-S" << std::endl;
