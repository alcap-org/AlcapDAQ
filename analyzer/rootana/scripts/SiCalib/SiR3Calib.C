...
void SiR3Calib() {
  ...
  double pulser_voltages[n_pulser_runs] = {0.05, 0.15, 0.20};
  double frac_pulser_uncertainty = 0.02; // estimate from the meeting
  ...
  for (int i_calib_point = 0; i_calib_point < n_calib_points; ++i_calib_point) {
    ...
    histname.str("");
    histname << "PlotTAP_Amplitude_wTimeCuts/hSiR3-S#MaxBinAPGenerator#{no_time_shift=true}_Amplitude_wTimeCuts";
    ...
    if ( (Int_t) result == 0) {
      ...
      std::cout << "SiR3-S (" << calib_energies[i_calib_point] << " +/- " << calib_energy_errors[i_calib_point] << " keV): " << calib_adcs[i_calib_point] << " +/- " << calib_adc_errors[i_calib_point] << " ADC (Esimate was " << estimate << ", NPeaks = " << spec.GetNPeaks() << ")" << std::endl;
  ...
  if ( (Int_t) result == 0) {
    ...
    std::cout << "SiR3-S: E [keV] = " << calib_gain << " (+/- " << calib_gain_error << ") * A [ADC] + " << calib_offset << " (+/- " << calib_offset_error << ")\t(chi^2/ndf = " << result->Chi2() << " / " << result->Ndf() << " = " << result->Chi2() / result->Ndf() << ")" << std::endl;
    ...
    std::cout << "Calibration curve fit failed for SiR3-S" << std::endl;
  }
  ...
  calib_script.open("scripts/r15b_insert_calib_sir3.sh", std::ofstream::out);
  calib_script << "#! /bin/bash" << std::endl;
  calib_script << std::endl;
  calib_script << "Channel=SiR3-S" << std::endl;
