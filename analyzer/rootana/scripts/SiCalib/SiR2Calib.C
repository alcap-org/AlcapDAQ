...
void SiR2Calib() {
  ...
  const int n_pulser_runs = 3;
  double pulser_voltages[n_pulser_runs] = {0.05, 0.15, 0.2};
  ...
  const int n_calib_points = 4;
  std::string calib_labels[n_calib_points] = { "Am241", "#splitline{50 mV Pulser}{@ 15 MeV/V}", "#splitline{150 mV Pulser}{@ 15 MeV/V}", "#splitline{200 mV Pulser}{@ 15 MeV/V}" };
  ...
  double calib_adcs[n_calib_points];
  double calib_adc_errors[n_calib_points];
  double calib_gain;
  double calib_gain_error;
  double calib_offset;
  double calib_offset_error;

  double pulser_energies[n_pulser_runs] = {mesytec*0.05, mesytec*0.15};
  double pulser_adcs[n_pulser_runs];
  double pulser_adc_errors[n_pulser_runs];
  ...
  for (int i_calib_point = 0; i_calib_point < n_calib_points; ++i_calib_point) {
    ...
    histname.str("");
    histname << "PlotTAP_Amplitude_wTimeCuts/hSiR2-S#MaxBinAPGenerator#{no_time_shift=true}_Amplitude_wTimeCuts";
    ...
    if ( (Int_t) result == 0) {
      calib_adcs[i_calib_point] = result->Parameter(1);
      calib_adc_errors[i_calib_point] = result->ParError(1);
      std::cout << "SiR2-S (" << calib_energies[i_calib_point] << " +/- " << calib_energy_errors[i_calib_point] << " keV): " << calib_adcs[i_calib_point] << " +/- " << calib_adc_errors[i_calib_point] << " ADC (Esimate was " << estimate << ", NPeaks = " << spec.GetNPeaks() << ")" << std::endl;
	...
	pulser_adcs[i_element] = result->Parameter(1);
	pulser_adc_errors[i_element] = result->ParError(1);
      }
    }
    else {
      std::cout << "SiR2-S, Calib Point #" << i_calib_point << " (" << calib_energies[i_calib_point] << " keV) fit failed" << std::endl;
    }
  ...
  si_channel_canvases->cd(n_calib_points+1);
  //    TCanvas* c1 = new TCanvas("c1", "c1");

  TGraphErrors* gr = new TGraphErrors(n_calib_points, calib_adcs, calib_energies, calib_adc_errors, calib_energy_errors);
  ...
  gr->GetYaxis()->SetRangeUser(-1000, 15*convert_to_keV);
  
  for (int i_calib_point = 0; i_calib_point < n_calib_points; ++i_calib_point) {
    TLatex* calib_text = 
      new TLatex( calib_adcs[i_calib_point]+100,
  ...
  if (n_calib_points == 1) {
    // single point calib
    calib_gain = calib_energies[0] / calib_adcs[0];
    calib_gain_error = 0;
    calib_offset = 0;
    calib_offset_error = 0;
    
    TF1* linear_fit = new TF1("linear_fit", "[0]*x + 1", 0, 4000);
    linear_fit->SetParameters(calib_gain, calib_offset);
  ...
  if ( (Int_t) result == 0) {
    ...
    calib_offset = result->Parameter(1);
    calib_offset_error = result->ParError(1);
    calib_gain = result->Parameter(0);
    calib_gain_error = result->ParError(0);
    
    std::cout << "SiR2-S: E [keV] = " << calib_gain << " (+/- " << calib_gain_error << ") * A [ADC] + " << calib_offset << " (+/- " << calib_offset_error << ")\t(chi^2/ndf = " << result->Chi2() << " / " << result->Ndf() << " = " << result->Chi2() / result->Ndf() << ")" << std::endl;
    //      result->PrintCovMatrix(std::cout);
    
    text.str("");
    text << "E [keV] = p0 * A [ADC] + p1";
    double first_line_y = 13*convert_to_keV;
    double line_step_y = 0.75*convert_to_keV;
    ...
    text.str("");
    text << std::fixed << std::setprecision(5) << "p0 = (" << calib_gain << " #pm " << calib_gain_error << ")";
    TLatex* latex2 = new TLatex(first_line_x, first_line_y-line_step_y, text.str().c_str());
    latex2->SetTextSize(0.04);
    latex2->Draw();
    
    text.str("");
    text << std::fixed << std::setprecision(2) << "p1 = (" << calib_offset << " #pm " << calib_offset_error << ")";
    ...
    std::cout << "Calibration curve fit failed for SiR2-S" << std::endl;
  }
  ...
  calib_script.open("scripts/r15b_insert_calib_sir2.sh", std::ofstream::out);
  calib_script << "#! /bin/bash" << std::endl;
  calib_script << std::endl;
  calib_script << "Channel=SiR2-S" << std::endl;
  calib_script << "Gain=" << calib_gain << std::endl;
  calib_script << "Offset=" << calib_offset << std::endl;
  ...
  TGraphErrors* gr2 = new TGraphErrors(n_pulser_runs, pulser_voltages, pulser_adcs, pulser_voltage_errors, pulser_adc_errors);
  gr2->SetMarkerStyle(kFullDotMedium);
  gr2->Draw("APE");
  gr2->GetXaxis()->SetTitle("Pulser Amplitude [mV]");
  gr2->GetYaxis()->SetTitle("Peak Position [ADC]");
  gr2->GetXaxis()->SetLimits(0, 0.25);
  gr2->GetYaxis()->SetRangeUser(0, 4000);
  
  gr2->Fit("pol1", "Q");
  
  TLine* line = new TLine(0, 0, pulser_voltages[1], pulser_adcs[1]);
  ...
  for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
    ...
    TFile* file = new TFile(filename.str().c_str(), "READ");
    TH1F* hist = (TH1F*) file->Get("PlotTAP_Amplitude/hSiR1-3-S#MaxBinAPGenerator#any_Amplitude");
