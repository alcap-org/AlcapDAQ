...
void Si16a_FinalPlot_2p1sFit_wTargetCoinc() {
  ...
  std::string norm_dir_name = ge_channel + "_wThinNoThickCoinc_MuonSlice500_1500_" + transition;
  ...
  //  std::string histtitle = "Si16a Dataset, Full X-Ray Spectrum (" + ge_channel + ")";

  
  RooPlot* Eframe = (ws->var("edep"))->frame();
  std::string histtitle = "Si16a Dataset, Run " + run_str.str() + ", X-Ray Spectrum, " + ge_channel + " (coincident with muon hit)";
