...
void SiL3_FinalPlot_CaptureRaySpectrum(std::string save_dir = "") {
  ...
  for (int i_ge_channel = 0; i_ge_channel < n_ge_channels; ++i_ge_channel) {
    ...
    std::string dirname = "XRaySpectrum_" + ge_channel + "_Capture1014keV_200nsTimeCut";
    ...
    for (int i_transition = min_transition; i_transition <= max_transition; ++i_transition) {
      ...
      xray.transition = "Capture1014keV";//transition_str.str();
    ...
    TPad* inset_pad = new TPad("inset", "inset", 0.45, 0.42, 0.92, 0.90);
