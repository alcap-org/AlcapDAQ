void SiL3_FinalPlot_Lifetime(std::string savedir = "") {
  ...
  const int n_energy_cuts = 4;
  double min_energy_cuts[n_energy_cuts] = {2000, 5000, 10000, 15000};
  ...
  for (int i_energy_cut = 0; i_energy_cut < n_energy_cuts; ++i_energy_cut) {
    ...
    TCanvas* c_Lifetime = new TCanvas();
    c_Lifetime->SetLogy();
    ...
    if (savedir != "") {
      std::string savename = savedir + "AlCapData_SiL3Dataset_ActiveTarget_Lifetime_" + std::to_string((int)min_energy_cut) + "keV";
