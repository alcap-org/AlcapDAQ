void SiL3_FinalPlot_TimeCutCorrection(std::string savedir = "", std::ostream& numbers_file = std::cout) {

  numbers_file << "% SiL3_FinalPlot_TimeCutCorrection.C" << std::endl;
  ...
  for (double i_min_time_slice = min_time_slice; i_min_time_slice < max_time_slice; i_min_time_slice += time_slice_step) {
    ...
    raw_spectrum->SetTitle("SiL3 Dataset, Active Target Analysis, Time Cut Correction");
    ...
    raw_spectrum->GetYaxis()->SetRangeUser(0.1,1e8);
  ...
  if (savedir != "") {
    std::string savename = savedir + "AlCapData_SiL3Dataset_ActiveTarget_TimeCutCorrection";
