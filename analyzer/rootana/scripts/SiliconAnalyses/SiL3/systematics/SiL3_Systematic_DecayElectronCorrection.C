...
void SiL3_Systematic_DecayElectronCorrection(std::string infilename, std::string outfilename,
				       std::string countfilename, std::string counttreename,
				       std::string time_slice_str, std::string recoil_name) {

  // Number of settings
  const int n_settings = 3;
  //  std::string settings[n_settings] = {"", "_KFactor0-9", "_KFactor0-85"};
  ...
  for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
    ...
    std::string corrfilename = "~/data/mc/SiL3/decayCorr_1M_Geom-P1_muplus_KFactor0-85_MeV" + setting + ".root";
    std::string corrhistname = "hEDep_muplus";
    outdirname = "DecayElectron_" + time_slice_str + "_" + recoil_name + "_DecayElec" + setting;
    ...
    inhistname = outdirname + "/hCorrectedSpectrum";
    outdirname = "FinalNormalisation_" + time_slice_str + "_" + recoil_name + "_DecayElec" + setting;
