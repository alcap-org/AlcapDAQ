...
Bool_t debug = kTRUE;
Double_t window = .06;
Double_t ymin = 1.57;//.26; //y-limits chosen for protons
Double_t ymax = 1.75;//1.75;//2.0;//1.4;
...
std::map<int, TH1D *> Slice(TH2D *h) {
	...
	for(int i=0; i < 7; i++) { //sliceIndexes Al50 SiR 35, SiL 30 //Al100 SiL 27 SiR 35
		//store projections of different Lg #DeltaE+E segments in map, divided into equal portions of 1MeV
	  float slice_width = 0.05;//0.25; // MeV
	  float min_slice = 0.0+(i*slice_width);//1.0+(i*slice_width);//(i+1)/2+(slice_width);
	  float max_slice = min_slice+slice_width;//(i+1)/2.+(2*slice_width);
	  std::cout << "sliceIndex " << i << ": " << min_slice << " -- " << max_slice << "" << std::endl;
	  map[i] = h->ProjectionY(Form("%.3f_%.3f_py", min_slice, max_slice), h->GetXaxis()->FindBin(min_slice ), h->GetXaxis()->FindBin(max_slice) );
...
void DefineCut(const char *sourceName, const char *species="proton", bool plotLeft=kFALSE) {
	TFile *fCut = new TFile("si16b-sir3-cuts.root", "UPDATE");
	...
	for(int sigma = 1; sigma < 5; ++sigma) {
		...
		for(int i=0; i < mapsize; i++) {
		  ...
		  float min_slice = 0.0+(i*slice_width);//1.0+(i*slice_width);//(i+1)/2+(slice_width);
		  float max_slice = min_slice+slice_width;//(i+1)/2.+(2*slice_width);
		  std::cout << "i = " << i<< ", " << min_slice << " -- " << max_slice << ", mean = " << results[meanStr][i] << std::endl;
		  //		  results[sigmaStr][i] = std::fabs(results[sigmaStr][i])
			if(i < mod) {
				cutg->SetPoint(2*i, min_slice, results[meanStr][i] - sigma*results[sigmaStr][i]);
				std::cout << "FW Point " << 2*i << ": " << min_slice << ", " << results[meanStr][i] - sigma*results[sigmaStr][i] << std::endl;
				cutg->SetPoint(2*i+1, max_slice, results[meanStr][i] - sigma*results[sigmaStr][i]);
				std::cout << "FW Point " << 2*i+1 << ": " << max_slice << ", " << results[meanStr][i] - sigma*results[sigmaStr][i] << std::endl;
			} else {
				cutg->SetPoint(2*i, min_slice, results[meanStr][mod-1] - sigma*results[sigmaStr][mod-1]);
				std::cout << "FW' Point " << 2*i << ": " << min_slice << ", " << results[meanStr][mod-1] - sigma*results[sigmaStr][mod-1] << std::endl;
				cutg->SetPoint(2*i+1, max_slice, results[meanStr][mod-1] - sigma*results[sigmaStr][mod-1]);
				std::cout << "FW' Point " << 2*i+1 << ": " << max_slice << ", " << results[meanStr][mod-1] - sigma*results[sigmaStr][mod-1] << std::endl;
			}
		}
		for(int i=mapsize; i < 2*mapsize; i++) {
			int c = 2*mapsize-i;
			float slice_width = 0.05;//0.25; // MeV
			float min_slice = 0.0+((c-1)*slice_width);//1.0+(i*slice_width);//(i+1)/2+(slice_width);
			float max_slice = min_slice+slice_width;//(i+1)/2.+(2*slice_width);
			std::cout << "c = " << c << ", i = " << i << " (" << min_slice << " -- " << max_slice << ")" << std::endl;

			if(c < mod) {
				cutg->SetPoint(2*i, max_slice, results[meanStr][c-1] + sigma*results[sigmaStr][c-1]);
				std::cout << "BW Point " << 2*i << ": " << max_slice << ", " << results[meanStr][c-1] + sigma*results[sigmaStr][c-1] << std::endl;
				cutg->SetPoint(2*i+1, min_slice, results[meanStr][c-1] + sigma*results[sigmaStr][c-1]);
				std::cout << "BW Point " << 2*i+1 << ": " << min_slice << ", " << results[meanStr][c-1] + sigma*results[sigmaStr][c-1] << std::endl;
			} else {
				cutg->SetPoint(2*i, max_slice, results[meanStr][mod-1-1] + sigma*results[sigmaStr][mod-1-1]);
				std::cout << "BW' Point " << 2*i << ": " << max_slice << ", " << results[meanStr][mod-1-1] + sigma*results[sigmaStr][mod-1-1] << std::endl;
				cutg->SetPoint(2*i+1, min_slice, results[meanStr][mod-1-1] + sigma*results[sigmaStr][mod-1-1]);
				std::cout << "BW' Point " << 2*i+1 << ": " << min_slice << ", " << results[meanStr][mod-1-1] + sigma*results[sigmaStr][mod-1-1] << std::endl;
			}
		}
		cutg->SetPoint(4*mapsize, 0.0, results[meanStr][0] - sigma*results[sigmaStr][0]);
		std::cout << "Fin. Point " << 4*mapsize << ": " << 0.0 << ", " << results[meanStr][0] - sigma*results[sigmaStr][0] << std::endl;
...
void Pid(const char *target="~/data/results/Si16b/subtrees_newPP_geq1TgtPulse_1.root", Bool_t plotLeft=kFALSE, const char *treeName="siBlockTree_SiR") {
	...
	TH2D *hLg_SiR_EvDeltaE = new TH2D("hLg_SiR_EvDeltaE", "SiR Ev#DeltaE;Lg E+#DeltaE / #sqrt{2} [MeV];Lg #DeltaE / #sqrt{2} [MeV]", 250, -0.1, 0.4, 50, ymin, ymax);
