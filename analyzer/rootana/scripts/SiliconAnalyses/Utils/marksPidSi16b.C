...
std::map<int, TH1D *> Slice(TH2D *h) {
	...
	for(int i=0; i < 40; i++) { //sliceIndexes Al50 SiR 35, SiL 30 //Al100 SiL 27 SiR 35
...
/**
  ...
  */
void PidSi16b(const char *filename="~/data/results/Si16b/subtrees_newPP_geq1TgtPulse_1.root", Bool_t plotLeft=kFALSE, const char *treeName="siBlockTree_SiR") {
	gStyle->SetOptFit(1);
