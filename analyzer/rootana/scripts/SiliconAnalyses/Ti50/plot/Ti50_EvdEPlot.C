...
void Ti50_EvdEPlot(std::string infilename, std::string outfilename) {
  ...
  double min_layer_coincidences[n_layer_coincidences] = {-10000, -500, 200};
  double max_layer_coincidences[n_layer_coincidences] = {10000, 500, 10000};

  const int n_arms = 2;
  std::string trees[n_arms] = {"SiR", "SiL"};
  int n_arm_channels[n_arms] = {4, 16};
    
  for (int i_arm = 0; i_arm < n_arms; ++i_arm) {
    std::string tree = trees[i_arm];
	...
	//    if (i_setting->name == "all") {
	int n_thin_channels = n_arm_channels[i_arm];
	for (int i_channel = 0; i_channel < n_thin_channels; ++i_channel) {
	  outdirname.str("");
	  outdirname << i_setting->name << "_" << tree << "1_" << i_channel+1 << "_" << layer_coincidence << "_TCutG";
	  args.outdirname = outdirname.str();
