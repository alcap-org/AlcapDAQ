...
void Ti50_RawSpectrum_fromEvdE(std::string infilename, std::string outfilename) {
  ...
  bool has_third_layer[n_arms] = {true, false};
  ...
  for (int i_arm = 0; i_arm < n_arms; ++i_arm) {
    ...
    bool third_layer = has_third_layer[i_arm];
	...
	for (int i_timecut = 0; i_timecut < n_timecuts; ++i_timecut) {
	  ...
	  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
	    ...
	    args.outdirname = outdirname + i_particle_name + "_" + pid + timecut + "_" + arm;
	    std::string indirname = i_particle_name + "_" + arm + "_timecut" + min_time.str() + "_10000ns_layerCoinc";
	    ...
	    if (i_particle_name == "proton" && third_layer) {
	      if (pid == "TCutG") {
