void Ti50_FinalPlot_Lifetime_forPaper() {
  ...
  std::string inhistname = "proton_SiR_timecut400_10000ns_layerCoinc/hThickTime_TwoLayer_12not3";
  ...
  hLifetime->SetTitle("Ti50 Dataset, Right Arm, Protons");
  hLifetime->SetStats(false);
  gStyle->SetOptFit(1);
  hLifetime->GetXaxis()->SetRangeUser(0, 10000);
  hLifetime->Draw("P E");

  TF1* fit_fn = new TF1("fit_fn", "[0]*TMath::Exp(-x/[1])+ [2]", 500, 10000);// + [2]");
  fit_fn->SetParameters(1000, 300, 0.1);
