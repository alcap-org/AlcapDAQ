void Ti50_FinalPlot_TwoLayer_Lifetime() {
  ...
  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
    ...
    std::string dirname = arm + "_" + this_particle + "_wTimeCut_SingleExp";
    ...
    std::string histtitle = "Ti50 Dataset, " + arm + ", Time";
    ...
    leglabel.str("");
    leglabel << this_particle << std::fixed << std::setprecision(1) << " (#tau = " << fit->GetParameter(1) << " #pm " << fit->GetParError(1) << ")";
