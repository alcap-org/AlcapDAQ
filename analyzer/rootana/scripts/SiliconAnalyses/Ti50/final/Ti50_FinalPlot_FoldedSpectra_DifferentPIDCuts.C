void Ti50_FinalPlot_FoldedSpectra_DifferentPIDCuts() {

  const int n_PIDs = 2;
  std::string PIDs[n_PIDs] = {"TCutG", "PSel"};
  std::string leglabels[n_PIDs] = {"Mark's PID (protons)", "John's PID (protons)"};
  ...
  //  std::string infilename = "~/data/results/Ti50/unfold_newPP.root";
  ...
  for (int i_PID = 0; i_PID < n_PIDs; ++i_PID) {
    ...
    std::string folded_histname = "proton_" + i_pid + "_SiR/hRawSpectrum";
