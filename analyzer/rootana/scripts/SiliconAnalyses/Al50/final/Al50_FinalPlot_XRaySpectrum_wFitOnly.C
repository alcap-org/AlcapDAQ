...
void Al50_FinalPlot_XRaySpectrum_wFitOnly(std::string savedir = "", std::ostream& numbers_file = std::cout) {
  ...
  double fit_bin_widths[n_transitions] = {0.5};

  numbers_file << "% From Al50_FinalPlot_XRaySpectrum_wFit.C" << std::endl;
  for (int i_transition = 0; i_transition < n_transitions; ++i_transition) {
    ...
    for (int i_ge_channel = 0; i_ge_channel < n_ge_channels; ++i_ge_channel) {
      std::string norm_file_name = "/home/edmonds/data/results/Al50/normalisation_forJohn.root";
	...
	return;
      ...
      std::string histtitle = "Al50 Dataset, Full X-Ray Spectrum (" + ge_channel + ")";
	...
	numbers_file << "Alternate";
      ...
      text.str("");
      text << dirname << "\'s Spectrum";
      std::cout << text.str() << std::endl;
      count->DrawLatexNDC(0.50, 0.85, text.str().c_str());
      
      if (savedir != "") {
	std::string savename = savedir + "AlCapData_Al50Dataset_FullXRaySpectrum_" + ge_channel;
