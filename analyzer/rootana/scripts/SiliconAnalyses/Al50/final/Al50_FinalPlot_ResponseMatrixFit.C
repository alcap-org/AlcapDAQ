...
void Al50_FinalPlot_ResponseMatrixFit(std::string savedir = "", std::ostream& numbers_file = std::cout) {
  ...
  double latex_xs[n_particles] = {0.68, 0.68, 0.68, 0.3};
  double latex_ys[n_particles] = {0.65, 0.65, 0.65, 0.55};
  ...
  std::string filename = "~/data/results/Al50/fits_fromMarkHists.root";
  ...
  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
	...
	std::cout << "T1 = " << fitted_fn->GetParameter(5) << " +/- " << fitted_fn->GetParError(5) << std::endl;
      ...
      std::string header = "AlCap Al - " + particle;
	...
	text << "N_{0} (exp(-T/T_{0}) + r exp(-T/T_{1}))";
      }
	...
	latex->DrawLatexNDC(latex_x, latex_y, text.str().c_str());	
      ...
      if (save_plots) {
	std::string pngname = basename + "/AlCapData_Al50Dataset_ResponseMatrixFit_" + fit + "Exp_" + particle + "_" + arm + ".png";
