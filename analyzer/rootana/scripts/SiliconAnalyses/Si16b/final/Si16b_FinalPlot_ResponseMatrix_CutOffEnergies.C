void Si16b_FinalPlot_ResponseMatrix_CutOffEnergies(std::string savedir = "") {
  ...
  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
    ...
    std::string infilename = "~/data/mc/Si16b/respMatrix_100M_Geom-P5_" + particle + "_100keVBins.root";
    //    std::string infilename = "~/data/mc/Si16b/respMatrix_10M_Geom-P5_" + particle + "_1MeVBins.root";
    std::string responsename = "SiR_";
    if (particle == "proton") {
      responsename += "three";
    }
    else {
    ...
    //    hTruth->Rebin(10);
    hTruth->Draw("HIST E");
    hTruth->SetXTitle("True Energy [MeV]");
