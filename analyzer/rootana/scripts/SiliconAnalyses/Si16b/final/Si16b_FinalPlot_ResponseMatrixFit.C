...
void Si16b_FinalPlot_ResponseMatrixFit(std::string savedir = "", std::ostream& numbers_file = std::cout) {

  bool save_plots = true;
  std::string basename = "~/plots/2021-06-24";
  ...
  std::string filename = "~/data/results/Si16b/fits_newPP_geq1TgtPulse_2.root";
  ...
  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
    ...
    std::string dirname = "FinalFit_SiR_" + particle;
    ...
    const int n_fits = 1;//4;
    std::string fits[n_fits] = {"Double"};//{"Single", "Double", "SingleThresh", "DoubleThresh"};
	...
	std::cout << "T1 = " << fitted_fn->GetParameter(5) << " +/- " << fitted_fn->GetParError(5) << std::endl;
      ...
      std::string header = "AlCap Si - " + particle;
	...
	latex->DrawLatexNDC(latex_x, latex_y, text.str().c_str());	
      ...
      if (save_plots) {
	std::string pngname = basename + "/AlCapData_Si16bDataset_ResponseMatrixFit_" + fit + "Exp_" + particle + "_" + arm + ".png";
  ...
  // leg->AddEntry(raw_spectrum, "raw spectrum", "lp");
  // leg->AddEntry(expo, "escape-corrected fit", "lp");
  //  leg->Draw();
