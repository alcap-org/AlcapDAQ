void Si16b_FinalPlot_TwoLayer_EvdE_TCutG_Systematic(std::string savedir = "", std::ostream& numbers_file = std::cout) {
  ...
  const int n_sigmas = 2;
  int sigmas[n_sigmas] = {2, 1};

  TCanvas* canvases[n_canvases][n_sigmas];
  for (int i_sigma = 0; i_sigma < n_sigmas; ++i_sigma) {
    std::string newcanvasname = "c_EvdE_" + std::to_string(sigmas[i_sigma]) + "sigma";
    canvases[0][i_sigma] = (TCanvas*) c_EvdE->Clone(newcanvasname.c_str());

    newcanvasname = "c_EvdE_incAlpha_" + std::to_string(sigmas[i_sigma]) + "sigma";
    canvases[1][i_sigma] = (TCanvas*) c_EvdE_incAlpha->Clone(newcanvasname.c_str());
  }
  ...
  for (int i_canvas = 0; i_canvas < n_canvases; ++i_canvas) {
    ...
    for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
      ...
      for (int i_sigma = 0; i_sigma < n_sigmas; ++i_sigma) {
	canvases[i_canvas][i_sigma]->cd();
	int sigma = sigmas[i_sigma];
      
	std::string tcutgname = this_particle + "_SiR_timecut0_10000ns_layerCoinc/hLg_SiR_EvDeltaE_" + this_particle + "_" + std::to_string(sigma) + "sigma_keV";
	TFile* sigfile = infile;
	if (sigma != 3) {
	  std::string sigfilename = "~/data/results/Si16b/plots_newPP_geq1TgtPulse_" + std::to_string(sigma) + "sigma.root";
	  sigfile = new TFile(sigfilename.c_str(), "READ");
	}
	TCutG* tCutG = (TCutG*) sigfile->Get(tcutgname.c_str());
	if (!tCutG) {
	  // try the other
  ...
  if (savedir != "") {
    std::string savename = savedir + "AlCapData_Si16bDataset_TwoLayer_EvdE_TCutG_Systematic";
    std::string pngname = savename + ".png";
    c_EvdE->SaveAs(pngname.c_str());
    
    savename = savedir + "AlCapData_Si16bDataset_TwoLayer_EvdE_incAlpha_TCutG_Systematic";
