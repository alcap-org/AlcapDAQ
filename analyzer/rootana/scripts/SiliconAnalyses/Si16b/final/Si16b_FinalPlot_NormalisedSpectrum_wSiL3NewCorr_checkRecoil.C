...
void Si16b_FinalPlot_NormalisedSpectrum_wSiL3NewCorr_checkRecoil(std::string savedir = "", std::ostream& numbers_file = std::cout) {
  ...
  for (int i_point = 0; i_point < pdta_spectrum->GetN(); ++i_point) {
      if (*(pdta_spectrum->GetX()+i_point) < 14.5
  ...
  self_leg->AddEntry(expo_clone, "AlCap active target", "lp");
  self_leg->AddEntry(spectrum, "AlCap passive target (p+d+t)", "lp");
  self_leg->AddEntry(pdta_spectrum, "AlCap passive target (p+d+t+#alpha)", "lp");
  self_leg->Draw();
  ...
  TLegend* leg_explanation = new TLegend(0.25,0.65,0.80,0.85);
  leg_explanation->SetBorderSize(0);
  leg_explanation->SetTextSize(0.04);
  leg_explanation->SetFillColor(kWhite);
  leg_explanation->SetHeader("AlCap Si");
  leg_explanation->AddEntry(SiL3_raw_spectrum, "AlCap active target (data)", "lp");
  leg_explanation->AddEntry(SiL3_fn, "AlCap active target (folded truth)", "lp");
  leg_explanation->AddEntry(expo, "AlCap active target (assumed truth)", "lp");
  ...
  IntegrateRates_wStatAndSystSpectra(grAlphas, grAlphas, 14, 15, inferRate, statErr, hiSystErr, loSystErr);
  std::cout << "Inferred Alphas (14 -- 15 MeV) = " << inferRate << std::endl;

  inferRate = 0; statErr = 0; hiSystErr = 0; loSystErr = 0;
  IntegrateRates_wStatAndSystSpectra(realAlphas, realAlphas, 15.0, 15.5, inferRate, statErr, hiSystErr, loSystErr);
  std::cout << "Real Alphas (15.0 -- 15.5 MeV) = " << inferRate << std::endl;
  inferRate = 0; statErr = 0; hiSystErr = 0; loSystErr = 0;
  IntegrateRates_wStatAndSystSpectra(realTritons, realTritons, 15.0, 15.5, inferRate, statErr, hiSystErr, loSystErr);
  std::cout << "Real Tritons (15.0 -- 15.5 MeV) = " << inferRate << std::endl;
  inferRate = 0; statErr = 0; hiSystErr = 0; loSystErr = 0;
  IntegrateRates_wStatAndSystSpectra(realDeuterons, realDeuterons, 15.0, 15.5, inferRate, statErr, hiSystErr, loSystErr);
  std::cout << "Real Deuterons (15.0 -- 15.5 MeV) = " << inferRate << std::endl;
  inferRate = 0; statErr = 0; hiSystErr = 0; loSystErr = 0;
  IntegrateRates_wStatAndSystSpectra(realProtons, realProtons, 15.0, 15.5, inferRate, statErr, hiSystErr, loSystErr);
  std::cout << "Real Protons (15.0 -- 15.5 MeV) = " << inferRate << std::endl;
  ...
  double min_energy = 15.0;//1.4;
  double max_energy = 15.5;//26;
