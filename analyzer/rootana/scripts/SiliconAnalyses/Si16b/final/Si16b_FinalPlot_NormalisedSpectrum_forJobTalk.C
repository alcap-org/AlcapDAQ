...
void Si16b_FinalPlot_NormalisedSpectrum_forJobTalk(std::string savedir = "", std::ostream& numbers_file = std::cout) {
  ...
  double Mu2e_cares_above[n_settings] = {3000, 4000, 5000, 12500};
  double arrow_ys[n_settings] = {1e-5, 6e-7, 2e-7, 1e-7};
  double arrow_latex_ys[n_settings] = {1e-5, 6e-7, 2e-7, 1e-7};
  //  double arrow_latex_ys[n_settings] = {2e-5, 1e-6, 1e-7, 5e-8};
  ...
  for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
    ...
    double arrow_y = arrow_ys[i_setting];
    TArrow* arrow = new TArrow(0, arrow_y, Mu2e_cares_above[i_setting], arrow_y, 0.02, "|>");
    arrow->SetLineColor(i_colour);
    arrow->SetFillColor(i_colour);
    arrow->SetLineWidth(2);
    arrow->SetLineStyle(kDashed);

    TLatex* arrow_latex = new TLatex();
    arrow_latex->SetTextAlign(22);
    arrow_latex->SetTextSize(0.03);
    arrow_latex->SetTextColor(i_colour);
    double arrow_latex_x = Mu2e_cares_above[i_setting]/2.0;
    double arrow_latex_y = arrow_latex_ys[i_setting];
    std::stringstream text;
    text.str("");
    text << "#splitline{" << leglabels[i_setting] << " stop}{in Mu2e target}";
    ...
    latex->DrawLatexNDC(0.6, 0.5, "AlCap #bf{#it{Preliminary} }");
    // arrow->Draw();
    // latex->DrawLatex(arrow_latex_x, arrow_latex_y, text.str().c_str());
