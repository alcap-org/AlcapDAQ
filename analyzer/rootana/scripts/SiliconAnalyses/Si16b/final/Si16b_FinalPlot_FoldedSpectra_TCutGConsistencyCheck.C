void Si16b_FinalPlot_FoldedSpectra_TCutGConsistencyCheck() {
  ...
  std::string infilename = "~/data/results/Si16b/unfold_newPP_geq1TgtPulse_noSiR3_1.root";
  ...
  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
    ...
    for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
      ...
      std::string folded_histname = "TimeCut_" + particle + "_TCutG_" + setting + "_layerCoinc500ns_tGT0ns/hInputSpectrum";
      //      std::string folded_histname = "TimeCut_" + particle + "_TCutG_" + setting + "_layerCoinc500ns_tGT0ns/hCorrectedSpectrum";
	...
	leglabel << setting << " (scaled by 1.0/" << std::fixed << std::setprecision(2) << scales[i_setting] << ")";
