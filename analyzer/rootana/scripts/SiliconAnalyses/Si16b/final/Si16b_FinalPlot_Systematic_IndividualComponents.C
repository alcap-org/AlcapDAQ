...
void Si16b_FinalPlot_Systematic_IndividualComponents(std::string savedir = "", std::ostream& numbers_file = std::cout) {
  ...
  const int n_systematics = 5;
  std::string systematics[n_systematics] = {"NIter", "PID", "MuStopPos", "SiRPos", "BinW"};
  double total_high_syst[n_systematics] = {0};
  double total_low_syst[n_systematics] = {0};
  ...
  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
	...
	//	Int_t i_colour = colours[i_setting];
      ...
      std::string i_dirname = "FinalSystPlot_" + systematic + "_" + particle + "_TCutG";
	...
	return;
      ...
      std::cout << particle << ": " << systematic << ": range " << min_best_energy[i_particle]/1000 << " MeV and " << max_best_energy[i_particle]/1000 << " MeV = +" << frac_high_syst_error << " -" << frac_low_syst_error << std::endl;
