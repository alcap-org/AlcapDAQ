void Si16b_FinalPlot_Systematic_MuonStopPositions(std::string savedir = "") {
  ...
  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
    ...
    if (savedir != "") {
      std::string savename = savedir + "AlCapData_Si16bDataset_NormalisedSpectrum_MuonStopPositions_" + Particle;
      std::string pdfname = savename + ".pdf";
      c_all_spectra->SaveAs(pdfname.c_str());
      std::string pngname = savename + ".png";
      c_all_spectra->SaveAs(pngname.c_str());

      savename = savedir + "AlCapData_Si16bDataset_Systematic_MuonStopPositions_" + Particle;
