void Si16b_FinalPlot_NormalisedSpectrum_wSiL3_RecoilComparison_forJobTalk(std::string savedir = "") {
  ...
  // std::string leglabels[n_recoils] = {"with recoil correction", "without recoil correction"};
  // Int_t colours[n_recoils] = {kRed, kBlue};

  const int n_recoils = 1;
  std::string recoils[n_recoils] = {"noRecoil"};
  std::string leglabels[n_recoils] = {"without recoil correction"};
  Int_t colours[n_recoils] = {kBlue};
  ...
  for (int i_recoil = 0; i_recoil < n_recoils; ++i_recoil) {
    ...
    SiL3_hist->GetXaxis()->SetRangeUser(0,17000);
    SiL3_hist->GetXaxis()->SetTitle("Energy [keV]");
    SiL3_hist->GetYaxis()->SetTitle("Particles / muon capture / keV");
    ...
    //    std::string leglabel = "SiL3 (active target, " + leglabels[i_recoil] + ")";
    std::string leglabel = "active Si target (" + leglabels[i_recoil] + ")";
    leg->AddEntry(SiL3_hist, leglabel.c_str(), "l");
  }
  ...
  for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
    ...
    leg->AddEntry(spectrum, leglabels_Si16b[i_setting].c_str(), "l");
  ...
  if (savedir != "") {
    std::string savename = savedir + "AlCapData_NormalisedSpectra_RecoilComparison_forJobTalk";
