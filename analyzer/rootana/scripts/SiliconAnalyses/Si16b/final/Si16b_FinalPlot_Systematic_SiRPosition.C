void Si16b_FinalPlot_Systematic_SiRPosition(std::string savedir = "") {
  ...
  std::string positions[n_settings] = {"", "_closerSiR", "_furtherSiR"};
  Int_t colours[n_settings] = {kBlack, kRed, kBlue};
  std::string leglabels[n_settings] = {"nominal", "1.5 mm closer SiR", "1.5 mm further SiR"};
  ...
  TLegend* leg2 = new TLegend(0.45,0.65,0.60,0.85);
  ...
  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
	...
	return;
      ...
      spectrum->SetTitle("Si16b Dataset, Muon Stop Positions");
    ...
    if (savedir != "") {
      std::string savename = savedir + "AlCapData_Si16bDataset_NormalisedSpectrum_SiRPosition_" + Particle;
      std::string pdfname = savename + ".pdf";
      c_all_spectra->SaveAs(pdfname.c_str());
      std::string pngname = savename + ".png";
      c_all_spectra->SaveAs(pngname.c_str());

      savename = savedir + "AlCapData_Si16bDataset_Systematic_SiRPosition_" + Particle;
