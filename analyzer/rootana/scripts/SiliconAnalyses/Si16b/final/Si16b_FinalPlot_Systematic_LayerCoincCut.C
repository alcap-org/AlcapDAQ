void Si16b_FinalPlot_Systematic_LayerCoincCut(std::string savedir = "") {
  ...
  std::string positions[n_settings] = {"central_hist", "syst_hist"};//, "Down", "Up"};
  Int_t colours[n_settings] = {kBlack, kRed};//, kRed, kBlue};
  std::string leglabels[n_settings] = {"nominal", "200ns"};//, "k = 0.975", "k = 1.025"};
  ...
  for (int i_particle = 0; i_particle < n_particles; ++i_particle) {
    ...
    for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
      std::string setting = positions[i_setting];
      Int_t i_colour = colours[i_setting];
    
      std::string i_dirname = "FinalNormalisation_" + particle + "_TCutG_2sig_layerCoinc500ns_tGT0ns_BinW500keV_LayerCoinc200ns_SystPlot";
      std::string i_histname = i_dirname + "/" + setting;
	...
	return;
      ...
      spectrum->SetTitle("Si16b Dataset, Layer Coincidence Cut");
    ...
    if (savedir != "") {
      std::string savename = savedir + "AlCapData_Si16bDataset_NormalisedSpectrum_LayerCoincSyst_" + Particle;
