void Si16b_FinalPlot_SiL3Inc_FlatBkgCorrection() {

  std::string filename = "~/data/results/Si16b/unfold.root";
  TFile* file = new TFile(filename.c_str(), "READ");

  const int n_slices = 1;
  double min_time_slice = 500;
  double max_time_slice = 15000;
  double time_slice_step = 14500;
  ...
  for (double i_min_time_slice = min_time_slice; i_min_time_slice < max_time_slice; i_min_time_slice += time_slice_step) {
    ...
    raw_spectrum->SetTitle("Si16b Dataset, SiL3 Inclusive Analysis, Flat Background Correction");
