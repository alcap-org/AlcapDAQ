void Si16b_FinalPlot_SiR3VetoDemo(std::string savedir = "", std::ostream& numbers_file = std::cout) {
  ...
  std::string histnames[n_hists] = {"hEvdE_TwoLayer_12not3", "hEvdE_ThreeLayer_123", "hEvdE_TwoLayer_123"};//, "hEvdE_ThreeLayer_123"};
  std::string histtitles[n_hists] = {"Si16b Dataset, Right Arm, 12#bar{3}", "Si16b Dataset, Right Arm, 123", "Si16b Dataset, Right Arm, 123"};//, "Si16b Dataset, Right Arm, With SiR3 Hit"};
  std::string uppertexts[n_hists] = {"Hits in SiR1 and SiR2 but not in SiR3 (our main measurement)", "Hits in SiR1, SiR2 and SiR3 (what we want to add after scaling up)", "Hits in SiR1, SiR2 and SiR3 (what we want to take away after scaling down)"};
  std::string cutdirnames[n_hists] = {"proton_2sig_SiR_timecut0_10000ns_layerCoinc500ns", "proton3L_2sig_SiR_timecut0_10000ns_layerCoinc500ns", "proton_2sig_SiR_timecut0_10000ns_layerCoinc500ns"};
  std::string cutnames[n_hists] = {"r_hLg_SiR_EvDeltaE_proton_2sigma_cutoff", "r_hLg_SiR_EvDeltaE_proton_2sigma", "r_hLg_SiR_EvDeltaE_proton_2sigma_cutoff"};

  std::string rawdirnames[n_hists] = {"proton_2sig_SiR_timecut0_10000ns_layerCoinc500ns", "proton3L_2sig_SiR_timecut0_10000ns_layerCoinc500ns", "proton_2sig_SiR_timecut0_10000ns_layerCoinc500ns"};
  std::string suffixes[n_hists] = {"_base", "_extension", "_reduction"};//withHitProtons", "threeLayer"};
  std::string leglabels[n_hists] = {"main measurement", "extension", "reduction"};
  int rebin_factors[n_hists] = {1, 1, 1};//10, 10};
  double max_x_axis[n_hists] = {25, 25, 25};//, 25};
  double max_y_axis[n_hists] = {3, 20, 3};//, 20};
  ...
  double sum_factors[n_hists] = {1, 1, -1};
