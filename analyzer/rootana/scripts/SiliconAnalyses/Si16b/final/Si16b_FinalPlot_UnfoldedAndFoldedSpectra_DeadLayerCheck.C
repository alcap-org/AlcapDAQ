void Si16b_FinalPlot_UnfoldedAndFoldedSpectra_DeadLayerCheck(std::string savedir = "", std::ostream& numbers_file = std::cout) {
  ...
  std::string settings[n_settings] = {"_0-5umDL", "_noDL", "_expo"};//"_10umDL"};//"_noDL_preCorr"};
  Int_t unfolded_colours[n_settings] = {kBlack, kRed, kBlue};//, kCyan, kMagenta, kSpring};
  std::string leglabels[n_settings] = {"unfolded with nominal response", "unfolded with no-dead-layer response", "unfolded with expo response"};//"unfolded with 10 #mum-dead-layer response"};//"pre-corrected and unfolded with no-dead-layer repsonse"};

  numbers_file << "% from Si16b_FinalPlot_UnfoldedAndFoldedSpectra.C" << std::endl;
  for (int i_particle = 0; i_particle < 1; ++i_particle) {
    TCanvas* c_Spectra = new TCanvas();

    TLegend* leg = new TLegend(0.45,0.60,0.75,0.85);
    //    TLegend* leg = new TLegend(0.15,0.15,0.65,0.45);
    leg->SetBorderSize(0);
    ...
    for (int i_setting = 0; i_setting < n_settings; ++i_setting) {
      ...
      std::string Particle = Particles[i_particle];
      std::string infilename = "~/data/results/Si16b/unfold_newPP_geq1TgtPulse" + settings[i_setting] + "_2.root";
      ...
      hFoldedSpectrum->SetLineColor(unfolded_colours[i_setting]);
      ...
      hUnfoldedSpectrum->SetLineColor(unfolded_colours[i_setting]);
	...
	round_precision = 2;
      ...
      // axis->SetLineColor(unfolded_colours[i_setting]);
      // axis->SetLabelColor(unfolded_colours[i_setting]);
      // axis->SetTitleColor(unfolded_colours[i_setting]);
