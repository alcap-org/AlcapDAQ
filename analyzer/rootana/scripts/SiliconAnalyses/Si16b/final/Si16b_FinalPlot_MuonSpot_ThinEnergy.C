void Si16b_FinalPlot_MuonSpot_ThinEnergy() {
  ...
  for (int i_cut = 0; i_cut < n_cuts; ++i_cut) {
    ...
    std::string newhistname = "hdE_" + cutnames[i_cut];
    TH1D* hdE = hEvdE->ProjectionY(newhistname.c_str());
    hdE->Rebin(5);
    hdE->GetXaxis()->SetRangeUser(0, 4000);

    axistitle.str("");
    axistitle << "Count / " << hdE->GetXaxis()->GetBinWidth(1) << " keV";
    hdE->SetStats(false);
    hdE->SetTitle("Si16b Dataset, Muon Spots, Thin Energy");
    hdE->SetYTitle(axistitle.str().c_str());
    
    hdE->SetLineColor(colours[i_cut]);
    hdE->Draw("HIST E SAME");

    leg->AddEntry(hdE, cutnames[i_cut].c_str(), "l");
  }
  leg->Draw();
