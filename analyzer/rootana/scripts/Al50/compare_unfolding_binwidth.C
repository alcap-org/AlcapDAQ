...
void compare_unfolding_binwidth(bool unfold_first=false) {
  if (unfold_first)
  ...
  hs[4][0]->SetTitle("Left;E [keV];Unfolded proton count/bin width");
  hs[4][1]->SetTitle("Right;E [keV];Unfolded proton count/bin width");
  for (int i = 0; i < NF; ++i) {
    for (int j = 0; j < 2; ++j) {
      hs[i][j]->Scale(1./norm);
  ...
  leg     ->Draw();
  c->SaveAs("img/compare_unfolding_binwidth_spec.png");
  ...
  mgrates->Draw("APL");
  lrates->Draw();
  crates->SaveAs("img/compare_unfolding_binwidth_rates.png");
  ...
  double ymean = std::accumulate(y[2].begin(), y[2].end(), 0.)/y[2].size();
  double ysig  = 0;
  for (int i = 0; i < NF; ++i)
