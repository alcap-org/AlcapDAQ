...
void compare_unfolding_iterations(bool unfold_first=false) {
  if (unfold_first)
  ...
  for (int i = 0; i < NF; ++i) {
    hs[i][0]->SetTitle("Left;E [keV];p/500 keV");
    hs[i][1]->SetTitle("Right;E [keV];p/500 keV");
    for (int j = 0; j < 2; ++j) {
      hs[i][j]->Scale(1./norm);
  ...
  mgrates->SetTitle("3.5-10 MeV Proton Rates;Unfolding Iterations;p/#mucap");
  ...
  leg     ->Draw();
  c->SaveAs("img/compare_unfolding_bayiter_spec.png");
  ...
  mgrates->Draw("APL");
  lrates->Draw();
  crates->SaveAs("img/compare_unfolding_bayiter_rates.png");
  ...
  double ymean = std::accumulate(y[2].begin(), y[2].end(), 0.)/NF;
  double dy    = y[2][3] - ymean;
