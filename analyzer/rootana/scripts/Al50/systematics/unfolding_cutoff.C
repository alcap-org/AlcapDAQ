...
void unfolding_cutoff(bool unfold_first=false) {
  ...
  for (int i = 0; i < N; ++i) {
    x[0][i] = x[1][i]  = 2.+i*0.5; // Where x-axis defines *************
    y[0][i] = n[i][0][0];
    y[1][i] = n[i][1][0];
  ...
  mgrates->SetTitle("3.5-10 MeV Proton Count;Lower Unfolding Bound [keV];Unfolded proton count");
  ...
  leg     ->Draw();
  c->SaveAs("img/systematics/compare_unfolding_cutoff_elo_spec.png");
  ...
  // hs[14][1]->Draw("SAME PLC");
  // hs[15][1]->Draw("SAME PLC");
  cspeczoom->SaveAs("img/systematics/compare_unfolding_cutoff_elo_spec_zoom.png");

  TCanvas* crates = new TCanvas("crates", "crates", 700, 500);
  mgrates->Draw("APL");
  crates->BuildLegend();
  crates->SaveAs("img/systematics/compare_unfolding_cutoff_elo_rates.png");
  ...
  double ymean[3] = {
    std::accumulate(y[0].begin(), y[0].end(), 0.)/N,
    std::accumulate(y[1].begin(), y[1].end(), 0.)/N,
    std::accumulate(y[2].begin(), y[2].end(), 0.)/N
  };
  double ysig[3]  = { 0, 0., 0. };
  for (int i = 0; i < N; ++i) {
