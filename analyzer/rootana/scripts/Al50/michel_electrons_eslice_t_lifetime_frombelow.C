{
  ...
  for (int i = 0; i < N; ++i) {
    ...
    hlt[i] = hlet[1]->ProjectionX(lname, hlet[1]->GetYaxis()->FindFixBin(e), hlet[1]->GetNbinsY());
    hrt[i] = hret[2]->ProjectionX(rname, hret[2]->GetYaxis()->FindFixBin(e), hret[2]->GetNbinsY());
  };
  for (int i = 0; i < N; ++i) {
  ...
  leg->AddEntry(hlt[0], "0keV<E");
  leg->AddEntry(hlt[1], "200keV<E");
  leg->AddEntry(hlt[2], "400keV<E");
  leg->AddEntry(hlt[3], "600keV<E");
  leg->AddEntry(hlt[4], "800keV<E");
  leg->AddEntry(hlt[5], "1000keV<E");
  leg->AddEntry(hlt[6], "1200keV<E");
  leg->AddEntry(hlt[7], "1400keV<E");
  leg->AddEntry(hlt[8], "1600keV<E");
  leg->AddEntry(hlt[9], "1800keV<E");
  ...
  for (int i = 0; i < N; ++i)
    hrt[i]->Draw("SAME");
  leg   ->Draw("SAME");
  ct->SaveAs("img/michel_electrons_eslice_lifetime_frombelow.png");
  ...
  TLegend* leglt = new TLegend(0.1, 0.7, 0.3, 0.9);
  leglt->AddEntry(gr[0], "SiL");
  leglt->AddEntry(gr[1], "SiR");
  TCanvas* clt = new TCanvas("clt", "clt", 700, 500);
  gr[0]->SetTitle("Lifetime after energy cut;Lower Bound [keV];Lifetime [ns]");
  gr[1]->SetTitle("Lifetime after energy cut;Lower Bound [keV];Lifetime [ns]");
  gr[0]->SetLineColor(kBlue);
  gr[1]->SetLineColor(kRed);
  gr[0]->Draw("AL");
  gr[1]->Draw("SAME L");
  leglt->Draw("SAME");
  gr[0]->GetYaxis()->SetRangeUser(850., 1200.);
  clt->SaveAs("img/michel_electrons_eslice_lifetime_graph_frombelow.png");
}
