...
void compare_tm_bound_cut() {
  ...
  RooUnfoldBayes* unf_0[2] = { new RooUnfoldBayes(resp[0], heraw[0], NITER),
                               new RooUnfoldBayes(resp[1], heraw[1], NITER) };
  h_e_reco[0][0] = unf_0[0]->Hreco();
  h_e_reco[0][1] = unf_0[1]->Hreco();
  TMUtils::SetRange(resp[0], 1e3, 14e3);
  TMUtils::SetRange(resp[1], 1e3, 14e3);
  FillMeasuredHists(resp, tr, heraw[0], heraw[1]);
  RooUnfoldBayes* unf_1[2] = { new RooUnfoldBayes(resp[0], heraw[0], NITER),
                               new RooUnfoldBayes(resp[1], heraw[1], NITER) };
  h_e_reco[1][0] = unf_1[0]->Hreco();
  h_e_reco[1][1] = unf_1[1]->Hreco();
  TMUtils::SetRange(resp[0], 2e3, 14e3);
  TMUtils::SetRange(resp[1], 2e3, 14e3);
  FillMeasuredHists(resp, tr, heraw[0], heraw[1]);
  RooUnfoldBayes* unf_2[2] = { new RooUnfoldBayes(resp[0], heraw[0], NITER),
                               new RooUnfoldBayes(resp[1], heraw[1], NITER) };
  h_e_reco[2][0] = unf_2[0]->Hreco();
  h_e_reco[2][1] = unf_2[1]->Hreco();
  TMUtils::SetRange(resp[0], 3e3, 14e3);
  TMUtils::SetRange(resp[1], 3e3, 14e3);
  FillMeasuredHists(resp, tr, heraw[0], heraw[1]);
  RooUnfoldBayes* unf_3[2] = { new RooUnfoldBayes(resp[0], heraw[0], NITER),
                               new RooUnfoldBayes(resp[1], heraw[1], NITER) };
  h_e_reco[3][0] = unf_3[0]->Hreco();
  h_e_reco[3][1] = unf_3[1]->Hreco();
  TMUtils::SetRange(resp[0], 4e3, 14e3);
  TMUtils::SetRange(resp[1], 4e3, 14e3);
  FillMeasuredHists(resp, tr, heraw[0], heraw[1]);
  RooUnfoldBayes* unf_4[2] = { new RooUnfoldBayes(resp[0], heraw[0], NITER),
                               new RooUnfoldBayes(resp[1], heraw[1], NITER) };
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    h_e_reco[i][0]->SetTitle("Modifying TM cutoff (Left);Energy (keV);Total proton count");
    h_e_reco[i][1]->SetTitle("Modifying TM cutoff (Right);Energy (keV);Total proton count");
  }
  TLegend* leg = new TLegend(0.7, 0.7, 0.9, 0.9);
  leg->SetHeader("TM Unfolding Range");
  leg->AddEntry(h_e_reco[0][0], "0-14 MeV");
  leg->AddEntry(h_e_reco[1][0], "1-14 MeV");
  leg->AddEntry(h_e_reco[2][0], "2-14 MeV");
  leg->AddEntry(h_e_reco[3][0], "3-14 MeV");
  leg->AddEntry(h_e_reco[4][0], "4-14 MeV");
  ...
  leg->Draw();
  c->SaveAs("img/compare_tm_bound_cut.png");
