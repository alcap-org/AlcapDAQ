#include "SiL3_PunchThroughPeak_Parameters.h"
...
void FitHistogram(RooWorkspace* ws, TH1F* hist, std::string suffix) {
  ...
  RooDataHist data(dataname.c_str(), "SiL3 Energy", (ws->argSet("energy")), hist);
  ws->import(data);
...
void DrawPlot(RooWorkspace* ws, TCanvas* c1, std::string suffix, int run_number, float scale_factor) {
  ...
  Eframe->Draw();
  histtitle.str("");
  histtitle << "SiL3 Energy (" << suffix << ", Run " << run_number << ", SF=" << scale_factor << ")";
...
void SiL3_PunchThroughPeak(int run_number, float scale_factor, bool save_pdfs=false) {
  ...
  //  histname << "hSiL3_Energy_wSiL1Coincidence";
  ...
  //  std::string this_suffix = "wSiL1Coinc";
  std::string this_suffix = "NoCuts";
