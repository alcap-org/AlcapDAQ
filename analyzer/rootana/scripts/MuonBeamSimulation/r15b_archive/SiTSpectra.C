void SiTSpectra() {
  ...
  for (int i_run = 0; i_run < n_total_runs; ++i_run) {
    ...
    for (int i_channel = 0; i_channel < n_SiT_channels; ++i_channel) {
      histname.str("");
      histname << "PlotTAP_Energy/hSiT-" << i_channel+1 << "-S#FirstComplete_RoughSyncPulseCut#{constant_fraction=0.20}{no_time_shift= true}_Energy;1";
	...
	newhistname << "hSiTSpectrum_" << i_run;
