...
void XYProfile() {
  ...
  RunInfo run10404(10404, is_horiz, in_vac, 45, 1.02, 4548, kBlack, "Run 10404, H, Vac, 45^{#circ}, SF=1.02", 1, 15);
  RunInfo run10465(10465, is_horiz, in_air, 45, 1.05, 1180, kRed, "Run 10465, H, Air, 45^{#circ}, SF=1.05", 1, 15);
  RunInfo run10473(10473, is_horiz, in_air, 90, 1.05, 1749, kBlue, "Run 10473, H, Air, 90^{#circ}, SF=1.05", 2, 15);
  RunInfo run10475(10475, is_vert,  in_air, 90, 1.05, 2858, kMagenta, "Run 10475, V, Air, 90^{#circ}, SF=1.05", 1, 15);
  RunInfo run10477(10477, is_vert,  in_vac, 90, 1.02, 2361, kSpring-5, "Run 10477, V, Vac, 90^{#circ}, SF=1.02", 1, 15);
  ...
  for (std::vector<RunInfo*>::const_iterator i_run = runs.begin(); i_run != runs.end(); ++i_run) {
    ...
    filename << "~/data/out/local/out" << (*i_run)->run_number << "_mu-beam-pos.root";
    //    filename << "~/data/out/local/out" << (*i_run)->run_number << "_mu-beam-pos_OldECut.root";
    ...
    hNMuonsPerStrip->SetMinimum(0);
    hNMuonsPerStrip->SetMaximum(80000);
    hNMuonsPerStrip->SetLineColor((*i_run)->colour);

    std::string fit_fn_name = "gaus";
    hNMuonsPerStrip->Fit(fit_fn_name.c_str(), "Q0", "", (*i_run)->fit_range_low, (*i_run)->fit_range_high);
    //        hNMuonsPerStrip->Fit(fit_fn_name.c_str(), "Q0", "", 2, 16);
    hNMuonsPerStrip->GetFunction(fit_fn_name.c_str())->SetLineColor((*i_run)->colour);
    double mean = hNMuonsPerStrip->GetFunction(fit_fn_name.c_str())->GetParameter(1);
    double mean_error = hNMuonsPerStrip->GetFunction(fit_fn_name.c_str())->GetParError(1);
    double sigma = hNMuonsPerStrip->GetFunction(fit_fn_name.c_str())->GetParameter(2);
    double sigma_error = hNMuonsPerStrip->GetFunction(fit_fn_name.c_str())->GetParError(2);
    double chi_sq = hNMuonsPerStrip->GetFunction(fit_fn_name.c_str())->GetChisquare();
    int ndf = hNMuonsPerStrip->GetFunction(fit_fn_name.c_str())->GetNDF();
    ...
    hNMuonsPerStrip->GetFunction(fit_fn_name.c_str())->Draw("LSAME");
