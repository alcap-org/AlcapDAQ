void SimpleMIPPeak() {
  ...
  const int n_runs = 8;
  int run_numbers[n_runs] = {8989, 8990, 8991, 8992, 8993, 8994, 8995, 8996};
  double scale_factors[n_runs] = {1.23, 1.24, 1.25, 1.26, 1.27, 1.28, 1.29, 1.30};
  ...
  for (int i_run = 0; i_run < n_runs; ++i_run) {
    ...
    TF1* func = new TF1(funcname.str().c_str(), "[0]*TMath::Gaus(x, [1], [2])", max_bin_center-2*energy_width, max_bin_center+2*energy_width);
    ...
    leglabel.str("");
    leglabel << std::fixed << std::setprecision(2) << "SF = " << i_scale_factor << ": MIP Peak = " << std::setprecision(0) << peaks[i_run] << " #pm " << peak_errors[i_run] << " keV";
