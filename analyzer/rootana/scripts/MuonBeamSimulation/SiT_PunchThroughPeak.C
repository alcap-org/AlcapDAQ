#include "SiT_PunchThroughPeak_Parameters.h"
...
void DrawPlot(RooWorkspace* ws, TCanvas* c1, std::string suffix, int run_number, float scale_factor) {
  ...
  Eframe->Draw();
  histtitle.str("");
  histtitle << "SiT Energy (" << suffix << ", Run " << run_number << ", SF=" << scale_factor << ")";
...
void SiT_PunchThroughPeak(int run_number, float scale_factor, bool save_pdfs=false) {
  ...
  //  filename << "muon-beam-hists_run" << std::setw(5) << std::setfill('0') << run_number << ".root";
  ...
  for (int i_si_channel = 0; i_si_channel < n_si_channels; ++i_si_channel) {
    histname.str("");
    histname << "PlotTAP_Energy/hSiT-" << i_si_channel+1 << "-S#FirstComplete_RoughSyncPulseCut#{constant_fraction=0.20}{no_time_shift= true}_Energy;1";
    TH1F* h = (TH1F*) file->Get(histname.str().c_str());
    if (!h) {
      std::cout << "Problem finding histogram " << histname.str() << std::endl;
    }
    else {
      if (!hEnergy) {
	hEnergy = (TH1F*) h->Clone("hEnergy");
      }
      else {
	hEnergy->Add(h);
      }
...
*/
  ...
  std::string this_suffix = "wNoCuts";
