void SiL1EDep_Run10477() {
  ...
  TH1D* sum_hist;
  for (int i_channel = 1; i_channel < 16; ++i_channel) {
    histname.str("");
    histname << "PlotTAP_EnergyTime/hSiL1-" << i_channel << "-S#FirstComplete_RoughSyncPulseCut#{constant_fraction=0.20}{no_time_shift= false}_EnergyTime";
