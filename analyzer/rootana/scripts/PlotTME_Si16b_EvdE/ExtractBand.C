...
int ExtractBand(std::string filename, TCut cuts, std::string dirname, std::string ntuplename, std::string x_varname, std::string y_varname) {
  ...
  for (std::vector<SiArm*>::iterator i_arm = arms.begin(); i_arm != arms.end(); ++i_arm) {
    ...
    arm_hits->Draw(drawcmd.c_str(), "", "COLZ");
    ...
    arm_hits->Draw(drawcmd.c_str(), cuts, "COLZ");
