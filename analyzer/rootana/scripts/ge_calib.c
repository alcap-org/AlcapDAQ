{
  ...
  double age = 13.8; // Years according to WolframAlpha
  double halflife = 13.537;
  double activity0 = 46.4e3; // RunPSI2013:661
  ...
  double livetime = 3245.5; // aAnalysis-R13:129
  double events = activity*livetime;

  // Spectrum
  const unsigned int NPeaks = 15;
  double Energy[NPeaks]    = {121.7817, 244.6975, 344.2785, 411.1163, 443.965, 510.999, 778.9040, 867.378, 964.079, 1085.869, 1089.737, 1112.074, 1212.948, 1299.140, 1408.006};
  double Intensity[NPeaks] = {0.2858,   0.07583,  0.265,    0.02234, 0.02821,  0.279,   0.12942,  0.04245, 0.14605, 0.10207,  0.01726,  0.13644,  0.01422,  0.01623,  0.21005};
  double ADC[NPeaks]       = {1000.75,  2015.48,  2831.37,  3379.41, 3648.90,  4196.38, 6392.91,  7118.00, 7909.78, 8907.53,  8941.08,  9121.85,  9948.85,  10654.3,  11547.5}; // First guess
  const char* Functions[NPeaks] = { "gaus(0)+[3]*x**2+[4]*x+[5]", 
				    "gaus(0)+gaus(3)+[6]*x+[7]",
				    "gaus(0)+gaus(3)+[6]*x+[7]",
				    "gaus(0)+[3]*x+[4]",
				    "gaus(0)+[3]*x+[4]",
  ...
  double LogEnergy[NPeaks];
  double ExpectedCounts[NPeaks];
  double Counts[NPeaks];
  double Efficiency[NPeaks];
  double LogEfficiency[NPeaks];
  ...
  const double rt2pi = TMath::Sqrt(2.*TMath::Pi());
  for (unsigned int i = 0; i < NPeaks; ++i) {
    ...
    TFitResultPtr res = spec->Fit(fit,"S");
    spec->DrawClone();
  ...
  // Draw
  TGraph* e_cal   = new TGraph(NPeaks, (Double_t*)ADC, (Double_t*)Energy);
  TGraph* eff_cal = new TGraph(NPeaks-1, (Double_t*)(LogEnergy+1), (Double_t*)(LogEfficiency+1));
  e_cal->SetTitle("Energy Calibration"); e_cal->GetXaxis()->SetTitle("ADC Value"); e_cal->GetYaxis()->SetTitle("Energy (keV)");
  eff_cal->SetTitle("Efficiency Calibration"); eff_cal->GetXaxis()->SetTitle("Ln[Energy (keV)]"); eff_cal->GetYaxis()->SetTitle("Ln[Counts / Expected Counts]");
  new TCanvas();
  e_cal->Fit("pol1"); e_cal->Draw("A*");
  new TCanvas();
  eff_cal->Fit("pol1"); eff_cal->Draw("A*");
}
