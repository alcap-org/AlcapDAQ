void ActiveTargetAnalysis() {
  ...
  //  std::string filename = "scripts/SiL3_ActiveTargetAnalysis/hist_files/PlotsFromTMETree_TwoRuns_Range-0-30MeV_Binning-100keV.root";
  ...
  // Set any bin outside of this to 0
  /*  for (int i_bin = 1; i_bin <= hTimeEnergy->GetXaxis()->GetNbins(); ++i_bin) {
    double bin_centre = hTimeEnergy->GetXaxis()->GetBinCenter(i_bin);
    if (bin_centre < min_time || bin_centre > max_time) {
      for (int j_bin = 1; j_bin <= hTimeEnergy->GetYaxis()->GetNbins(); ++j_bin) {
	int global_bin = hTimeEnergy->GetBin(i_bin, j_bin);
	hTimeEnergy->SetBinContent(global_bin, 0);
      }
    }
  }
  */

  double min_time_flat_bkg = 12000;
  ...
  std::cout << "AE: " << muon_time_cut << std::endl;
  ...
  */
  ...
  spectrum_fit->SetParameters(hEnergyCorrected->GetMaximum(), 1000);
  hEnergyResult->Fit(spectrum_fit, "R");
  ...
  double error;
  double n_protons = hEnergyResult->IntegralAndError(min_energy_bin, max_energy_bin, error);
