void StoppedMuonDiscrepancy(int run_to_analyse) {
  ...
  if (run_to_analyse > 0) {
    ...
    histtitle_suffix << " (Full Si16b Dataset, Ge Eff from Run " << ge_eff_from_run << ")";
  }
  int n_runs=1;
  ...
  //  xray.transition = "3p-1s";
  //  xray.energy = 476.80;
  //  xray.intensity = 0.0740;
  //  xray.intensity_error = 0.0020;
  //  xray.transition = "4p-1s";
  //  xray.energy = 503.58;
  //  xray.intensity = 0.0427;
  //  xray.intensity_error = 0.0020;  
  // 5p-1s too close to 511 keV
  //  xray.transition = "6p-1s";
  //  xray.energy = 522.74;
  ...
  // For 4p-1s
  //  double xray_energy_low = xray.energy-10;
  //  double xray_energy_high = xray.energy+2;
  // For 6p-1s
  //  double xray_energy_low = xray.energy-4;
  //  double xray_energy_high = xray.energy+10;


  std::stringstream filename;
  if ( (run_to_analyse > 0 && run_to_analyse < 9735) || (run_to_analyse > 9743 && run_to_analyse < 10362) ) {
    filename << "/gpfs/home/edmonds_a/AlcapDAQ/analyzer/rootana/out" << std::setw(5) << std::setfill('0') << run_to_analyse << "_coincidences.root";
  }
  else if (run_to_analyse >= 9735 && run_to_analyse <= 9743) {
    filename << "/gpfs/home/edmonds_a/data/out/v19/out" << std::setw(5) << std::setfill('0') << run_to_analyse << ".root";
  }
  else if (run_to_analyse >= 10362) {
    filename << "/gpfs/home/edmonds_a/data/out/v16/out" << std::setw(5) << std::setfill('0') << run_to_analyse << ".root";
  }
  else {
    filename << "/gpfs/home/edmonds_a/data/out/v16/Si16b.root";
  }
  ...
  filename.str("");
  if (run_to_analyse > 0) {
    filename << "stopped-muon-discrepancy-plots_run" << std::setw(5) << std::setfill('0') << run_to_analyse << "_eff" << std::setw(5) << std::setfill('0') << ge_eff_from_run << ".root";
  }
  else {
    filename << "stopped-muon-discrepancy-plots_Si16b_eff" << std::setw(5) << std::setfill('0') << ge_eff_from_run << ".root";
  }
  TFile* out_file = new TFile(filename.str().c_str(), "RECREATE");
  ...
  const int n_bins = 2;
  //  double bin_edges[n_bins+1] = {0, 400, 600, 700, 800, 1000, 1200, 8000};
  double bin_edges[n_bins+1] = {0,10000};
  TH1F* hRatioPerSiL1EnergyCut = new TH1F("hRatioPerSiL1EnergyCut", "", n_bins,bin_edges);
  ...
  for (int i_bin = 1; i_bin <= n_bins; ++i_bin) {
    ...
    double min_sil1_muon_energy = hRatioPerSiL1EnergyCut->GetXaxis()->GetBinLowEdge(i_bin);
    double max_sil1_muon_energy = hRatioPerSiL1EnergyCut->GetXaxis()->GetBinUpEdge(i_bin);
    ...
    gPad->Update();
    double coinc_cut_low_SiL3_SiL1 = hCuts->GetBinContent(hCuts->GetXaxis()->FindBin("SiL3-SiL1 (Low)"))/n_runs;
    ...
    double coinc_cut_high_SiL3_SiL1 = hCuts->GetBinContent(hCuts->GetXaxis()->FindBin("SiL3-SiL1 (High)"))/n_runs;
    ...
    gPad->Update();
    double coinc_cut_low_SiL3_GeLoGain = hCuts->GetBinContent(hCuts->GetXaxis()->FindBin("SiL3-GeLoGain (Low)"))/n_runs;
    ...
    double coinc_cut_high_SiL3_GeLoGain = hCuts->GetBinContent(hCuts->GetXaxis()->FindBin("SiL3-GeLoGain (High)"))/n_runs;
    ...
    gPad->Update();
    double coinc_cut_low_SiL1_GeLoGain = hCuts->GetBinContent(hCuts->GetXaxis()->FindBin("SiL1-GeLoGain (Low)"))/n_runs;
    ...
    double coinc_cut_high_SiL1_GeLoGain = hCuts->GetBinContent(hCuts->GetXaxis()->FindBin("SiL1-GeLoGain (High)"))/n_runs;
	...
	integral_high = max_sil3_muon_energy;
      ...
      double error = 0;
      TH1F* h_det_ToIntegrate = h_det_AfterCut;
      if (h_det_ToIntegrate->GetEntries() == 0) {
	h_det_ToIntegrate = h_det_BeforeCut; // there were no SiL3-SiL1 coincidences (either because there was no SiL1 or whatever)
      }
      double integral = h_det_ToIntegrate->IntegralAndError(low_integral_bin, high_integral_bin, error);
      ...
      TLine* low_muon_energy_cut_line = new TLine(integral_low, 0, integral_low, h_det_ToIntegrate->GetBinContent(low_integral_bin));
      low_muon_energy_cut_line->SetLineColor(h_det_ToIntegrate->GetLineColor());
      low_muon_energy_cut_line->SetLineStyle(kDashed);
      low_muon_energy_cut_line->SetLineWidth(2);
      low_muon_energy_cut_line->Draw("LSAME");

      TLine* high_muon_energy_cut_line = new TLine(integral_high, 0, integral_high, h_det_ToIntegrate->GetBinContent(high_integral_bin));
      high_muon_energy_cut_line->SetLineColor(h_det_ToIntegrate->GetLineColor());
      ...
      h_det_BeforeCut_ZoomIn->GetXaxis()->SetRangeUser(0,max_energy_zoomin);
      ...
      h_det_AfterCut_ZoomIn->GetXaxis()->SetRangeUser(0,max_energy_zoomin);
      h_det_AfterCut_ZoomIn->Draw("HIST E SAME");
      low_muon_energy_cut_line->Draw("LSAME");
      high_muon_energy_cut_line->Draw("LSAME");

      TArrow* integral_arrow = new TArrow(integral_low, 10, integral_high, 10, 0.02, "<->");
      integral_arrow->SetLineColor(h_det_ToIntegrate->GetLineColor());
      ...
      c_GeLoGain_det->cd(1);
      hGeLoGain_det_CoincidentEnergies->GetXaxis()->SetRangeUser(0,max_energy_zoomin);
      hGeLoGain_det_CoincidentEnergies->GetYaxis()->SetRangeUser(0,max_energy_zoomin);
      ...
      hGeLoGain_CoincidentEnergies->GetXaxis()->SetRangeUser(0,max_energy_zoomin);
      ...
      h_det_CoincidentEnergies->GetXaxis()->SetRangeUser(0,max_energy_zoomin);
      ...
      double first_line_x = xray.energy-5;
      gPad->Update();
    ...
    int i_bin = hRatioPerSiL1EnergyCut->FindBin(min_sil1_muon_energy);
    double bin_content = hRatio->GetBinContent(hRatio->GetXaxis()->FindBin("SiL1"));
    double bin_error = hRatio->GetBinError(hRatio->GetXaxis()->FindBin("SiL1"));
    if (bin_content > 0) {
      hRatioPerSiL1EnergyCut->SetBinContent(i_bin, bin_content);
      hRatioPerSiL1EnergyCut->SetBinError(i_bin, bin_error);
    }
  }
