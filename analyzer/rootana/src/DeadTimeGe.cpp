...
int DeadTimeGe::ProcessEntry(TGlobalData *gData, TSetupData *gSetup){
  ...
  std::vector<TAnalysedPulse*> detA_pulses = gAnalysedPulseMap[detname];
  std::vector<TAnalysedPulse*>::iterator currentDetAPulse = detA_pulses.begin(); 
	...
	for (std::vector<TAnalysedPulse*>::iterator detAPulseIter = currentDetAPulse;
