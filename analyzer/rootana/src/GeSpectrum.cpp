...
GeSpectrum::GeSpectrum(modules::options* opts) :
  ...
  fPileupProtected(opts->GetBool("pu_protect")),
  fChargedParticleProtected(opts->GetBool("cp_protect")),
  fUseSlowTiming(opts->GetBool("ge_slow_timing")),
  fCalibration(opts->GetBool("calib")),
  ...
  fMBAmpGe(SetupNavigator::Instance()->GetPedestal(fGeS),
           TSetupData::Instance()->GetTriggerPolarity(TSetupData::Instance()->GetBankName(fGeS.str()))),
  ...
  fTimeWindow_Small(opts->GetDouble("tw_small")),
  fTimeWindow_Big(opts->GetDouble("tw_big")), fTimeWindow_Asymmetric(),
  fPileupProtectionWindow(opts->GetDouble("pu_window")),
  fChargedParticleProtectionWindow(opts->GetDouble("cp_window")),
  ...
  if (!fCalibration) {
    ...
    for (unsigned int i = 0; i < fvPeaksOfInterest.size(); ++i) {
      ...
      const double bounds[2] = { fvPeaksOfInterest[i] - 2.,
                                 fvPeaksOfInterest[i] + 2. };
      sprintf(name,  "hTimePeak%ld", ::lround(fvPeaksOfInterest[i]));
      sprintf(title, "Timing around %ld keV Peak",
              ::lround(fvPeaksOfInterest[i]));
      fvhTimePeak.push_back(new TH2D(name, title, 2000, -5000., 5000., 32,
                                     bounds[0], bounds[1]));
    }
  }
}
