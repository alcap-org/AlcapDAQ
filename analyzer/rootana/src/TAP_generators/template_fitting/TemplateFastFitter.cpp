...
int TemplateFastFitter::FitWithOneTimeFree(int index, const TH1D* hPulse){
...
int TemplateFastFitter::FitWithAllTimesFixed( const TH1D* hPulse){
  // Prepare for minimizations
  fMinuitFitter->Clear();
  fFitFCN->SetPulseHist(hPulse);
  fMinuitFitter->CreateMinimizer(TFitterMinuit::kMigrad);
...
void TemplateFastFitter::AddTemplate(TTemplate* tpl){
