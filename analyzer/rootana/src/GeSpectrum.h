...
class GeSpectrum : public BaseModule {
 ...
 private:
  ...
  std::vector<double> CalculateTimes(const IDs::channel& chan,
                                     const std::vector<TPulseIsland*>& tpis);
  /// \brief
  /// Takes a vector of TPIs and calculates all of their energies.
  std::vector<double> CalculateEnergies(const IDs::channel& chan,
                                        const std::vector<TPulseIsland*>& tpis);
  /// \brief
  /// Removes small muSc pulses.
  void RemoveSmallMuScPulses(std::vector<double>& time,
                             std::vector<double>& energy);
  /// \brief
  /// Removes pileup muons.
  void RemovePileupMuScPulses(std::vector<double>& time,
                              std::vector<double>& energy);
  void RemoveChargedParticles(std::vector<double>& time,
                              std::vector<double>& energy,
                              const std::vector<double>& scint);

  static void SaveInputsAndThrowIfInputsInsane(const modules::options*);
  static void ThrowIfGeInsane(const std::vector<TPulseIsland*>& ge_fasts,
