...
PlotTME_muStops::PlotTME_muStops(modules::options* opts):
   ...
   fStoppedMus(0), fNStopsThisEvent(0), fEventNo(1),
...
int PlotTME_muStops::BeforeFirstEntry(TGlobalData* gData,const TSetupData *setup){
    ...
    fTDiffVsAmpSiR2=new TH2F("hTDiffVsAmpSiR2",
                             "Amplitudes of SiR2 hits compared to muSc (no pile-up) against the time diference",
    ...
    fAmplitudes=new TH2F("hAmp2d", "Amplitudes of SiR2 hits compared to muSc (with pile-up)", 4000, 0,4000, 4000,0,4000);
    fAmplitudes->SetXTitle("Amplitude in muSc (ADC units)");
    fAmplitudes->SetYTitle("Amplitude in SiR2 (ADC units)");
    fAmplitudes_PP=new TH2F("hAmp2d_PP", "Amplitudes of SiR2 hits compared to muSc (pile-up protected)", 4000, 0,4000, 4000,0,4000);
    ...
    fStopsPerEvent=new TH1F("hStoppedPerEvent","Number of #mu stops per midas event",
...
void PlotTME_muStops::FillHistograms(const TMuonEvent* tme, const IDs::source& muSc_source, const IDs::source& sir2_source){
  ...
  for(int i=0; i< N_sir2; ++i){
    ...
    double t_diff=sir2_tdp->GetTime(TDetectorPulse::kFast) - time;
    double sir2_amp=sir2_tdp->GetAmplitude(TDetectorPulse::kFast);
    fTDiff->Fill(t_diff);
...
ALCAP_REGISTER_MODULE(PlotTME_muStops);
