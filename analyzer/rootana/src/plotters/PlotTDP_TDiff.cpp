...
PlotTDP_TDiff::PlotTDP_TDiff(modules::options* opts):
   BaseModule("PlotTDP_TDiff",opts),fCutHists(false){
...
int PlotTDP_TDiff::BeforeFirstEntry(TGlobalData* gData,const TSetupData *setup){
    ...
    if(!makeTDPs){
        cout<<"It's meaningless to use the PlotTDP_TDiff module without MakeDetectorPulses as well"<<endl;
        return 1;
    }
    // Make sure MakeDetectorPulses is called before PlotTDP_TDiff
...
int PlotTDP_TDiff::AfterLastEntry(TGlobalData* gData,const TSetupData *setup){
    ...
    for(PlotsList_t::iterator i_source=fPlotsList.begin();
        ...
        if(((int)fit)!=0){
            cout<<"PlotTDP_TDiff: Warning: fitting to 2D amplitude plots failed with status: "
    ...
    if(Debug()){
        cout<<"PlotTDP_TDiff: Fitted amplitude gradients:"<<endl;
...
ALCAP_REGISTER_MODULE(PlotTDP_TDiff,slow_ch_cut,fast_ch_cut);
