...
Contents:     calculates absolute time withing MIDAS block for DT5730
...
std::map< std::string, std::vector<double> > dt5730_time_map;

static map<std::string, TH1D*> h1_time_map;            // Time in MIDAS block in ns

ANA_MODULE MDT5730_time_module =
{
	"MDT5730_time",                /* module name           */
...
INT module_init() 
{
  ...
  TDirectory *dir = dir0->mkdir("MDT5730_time");
  dir->cd();
  ...
  for (unsigned int i=0; i<NCHAN; i++)
    {
      ...
      vector<double> v;
      dt5730_time_map[bankname] = v;
      
      TH1D *h1_time = new TH1D(Form("h1_time_%s",bankname.c_str()),Form("time in block, %s",bankname.c_str()),16384,0.0,120.0);
...
INT module_event(EVENT_HEADER *pheader, void *pevent) 
{
  ...
  for (unsigned int i=0; i<NCHAN; i++)
    {
      std::string bankname( Form("D7%02d",i) );
      dt5730_time_map[bankname].clear();
    }
  ...
  for (map_iterator theMapIter = pulses_map.begin(); theMapIter != pulses_map.end(); theMapIter++) 
    {
      ...
      */
      ...
      if ( dt5730_defna_map.find(bankname) != dt5730_defna_map.end() && dt5730_time_map.find(bankname) !=  dt5730_time_map.end() )
	{
	  ...
	  for (unsigned int i=0; i<thePulses.size(); i++)
	    {
	      ...
	      if ( defna[i].E >  0 )
		{
		  t = (defna[i].t0 + thePulses[i]->GetTimeStamp())*2.0; // time in ns 
		  if (  h1_time_map[bankname] )
		    {
		      h1_time_map[bankname]->Fill( t*1.0e-6 );
		    }
		}
	      dt5730_time_map[bankname].push_back( t );
	    } 
	}
    }
