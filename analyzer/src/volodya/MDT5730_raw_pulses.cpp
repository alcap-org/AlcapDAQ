...
Name:         MDT5730_raw_pulses
Created by:   Volodya Tishchenko

Contents:     raw pulse histograms for DT5730
...
static map<std::string, TGraph*> gr_nislands_map;    // number of islands vs midas segment

ANA_MODULE MDT5730_raw_pulses_module =
{
	"MDT5730_raw_pulses",          /* module name           */
...
INT module_bor(INT run_number) {
  ...
  TDirectory *dir = dir0->mkdir("MDT5730_raw_pulses");
  dir->cd();
  ...
  for (unsigned int i=0; i<NCHAN; i++)
    {
      ...
      TGraph *gr_nislands = new TGraph();
      gr_nislands->SetName( Form("gr_nislands_%s",bankname.c_str()) );
      dir->Append( gr_nislands );
      gr_nislands_map[bankname] = gr_nislands;
      ...
      */
      ...
      TH2D *h2_pulses = new TH2D(Form("h2_pulses_%s",bankname.c_str()),Form("ADC vs clock tick, %s",bankname.c_str()),300,-0.5,299.5,16384,-0.5,16383.5);      
...
INT module_event(EVENT_HEADER *pheader, void *pevent) 
{
  ...
  for (map_iterator theMapIter = pulses_map.begin(); theMapIter != pulses_map.end(); theMapIter++) 
      ...
      for (unsigned int i=0; i<thePulses.size(); i++)
	{
      ...
      TGraph *gr_nislands = gr_nislands_map[bankname];
      if ( gr_nislands )
	{
	  Int_t np = gr_nislands->GetN();
	  gr_nislands->SetPoint(np, pheader->serial_number, thePulses.size());
	}
