...
Contents:     defna pulse analysis for DT5730
...
std::map< std::string, std::vector<TDefna> > dt5730_defna_map;

//static map<std::string, TH2D*> h2_pulses_map;       // ADC vs time histograms


ANA_MODULE MDT5730_defna_module =
{
	"MDT5730_defna",               /* module name           */
  ...
  for (unsigned int i=0; i<NCHAN; i++)
    {
      ...
      dt5730_defna_map[bankname] = v_defna;
    }

  // special channels
  defna_param_map["D702"].polarity = 1;
  defna_param_map["D706"].polarity = 1;
  defna_param_map["D707"].polarity = 1;
...
INT module_event(EVENT_HEADER *pheader, void *pevent) 
{
  ...
  for (unsigned int i=0; i<NCHAN; i++)
    {
      std::string bankname( Form("D7%02d",i) );
      dt5730_defna_map[bankname].clear();
    }
      ...
      if ( defna_param_map.find(bankname) != defna_param_map.end() )
	{
	  ...
	  for (unsigned int i=0; i<thePulses.size(); i++)
	    {	      
	      ...
	      dt5730_defna_map[bankname].push_back( defna );
