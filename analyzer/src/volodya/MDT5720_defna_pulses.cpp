...
Name:         MDT5720_defna_pulses
Created by:   Volodya Tishchenko

Contents:     defna pulse histograms for DT5720
...
ANA_MODULE MDT5720_defna_pulses_module =
{
	"MDT5720_defna_pulses",        /* module name           */
...
INT module_bor(INT run_number) {
  ...
  TDirectory *dir = dir0->mkdir("MDT5720_defna_pulses");
  dir->cd();
  ...
  for (unsigned int i=0; i<NCHAN; i++)
    {
      ...
      TH2D *h2_pulses = new TH2D(Form("h2_pulses_%s",bankname.c_str()),Form("ADC vs clock tick, %s",bankname.c_str()),600,-300.5,300.5,4096,-0.5,4095.5);      
...
INT module_event(EVENT_HEADER *pheader, void *pevent) 
{
  ...
  //printf("MDT5720_defna(): event %06i ==================================== \n",pheader->serial_number);
  ...
  for (map_iterator theMapIter = pulses_map.begin(); theMapIter != pulses_map.end(); theMapIter++) 
    {
      ...
      */
      ...
      if ( dt5720_defna_map.find(bankname) == dt5720_defna_map.end() ) continue;
