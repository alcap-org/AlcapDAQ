...
/*-- Module declaration --------------------------------------------*/
INT  MScPulseTime_init(void);
INT  MScPulseTime(EVENT_HEADER*, void*);
...
ANA_MODULE MScPulseTime_module =
{
	"MScPulseTime",                    /* module name           */
	"Andrew Edmonds",              /* author                */
	MScPulseTime,                      /* event routine         */
	NULL,                          /* BOR routine           */
	NULL,                          /* EOR routine           */
	MScPulseTime_init,                 /* init routine          */
	NULL,                          /* exit routine          */
 ...
 */
INT MScPulseTime(EVENT_HEADER *pheader, void *pevent)
{
	...
	// Loop over the map and sct each bankname, vector pair
			...
			simpleScMapIter++) 
	{
		...
		// Loop over the TSimpleScPulses and plot the histogram
		for (std::vector<TSimpleScPulse*>::iterator scPulse = theScPulses.begin(); scPulse != theScPulses.end(); scPulse++) {
			...
			else
			{
				...
				DetectorToTimeHistMap[detname]->Fill((*scPulse)->GetPulseTime() * GetClockTickForChannel(bankname));
