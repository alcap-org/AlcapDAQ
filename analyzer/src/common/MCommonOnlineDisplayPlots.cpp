...
INT MCommonOnlineDisplayPlots_init()
{
  ...
  for(std::map<std::string, std::string>::iterator mapIter = bank_to_detector_map.begin(); 
    ...
    TH1I* hPulseHeights = new TH1I(histname.c_str(), histtitle.c_str(), 4095,0,4095);
    ...
    hPulseTimestamps->SetBit(TH1::kCanRebin);
	...
	for (map_iterator theMapIter = pulse_islands_map.begin(); theMapIter != pulse_islands_map.end(); theMapIter++) 
	{
	  ...
	  for (std::vector<TPulseIsland*>::iterator thePulseIter = thePulses.begin(); thePulseIter != thePulses.end(); thePulseIter++) {
	    ...
	    if (timestamp_histograms_map.find(bankname) != timestamp_histograms_map.end())
	      timestamp_histograms_map[bankname]->Fill((*thePulseIter)->GetTimeStamp());
	  }
	}
	return SUCCESS;
}
