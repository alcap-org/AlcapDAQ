...
INT MIdentifySyncPulseRedux_init() {
  ...
  for (int icrate = 0; icrate < NCRATE; ++icrate) {
    ...
    // ntuple
    string ntupname = "ntup_"+std::to_string(icrate)+"_SyncPulse";
    string ntuptitle = "ntuple for identified sync pulses in crate "+std::to_string(icrate);
    TNtuple* ntuple = new TNtuple(ntupname.c_str(), ntuptitle.c_str(), "Event:SyncPulseIndex:SyncPulseOffset");
    // turn off autosave
    ntuple->SetAutoSave(0);
    ntupIdentifySyncPulse[icrate] = ntuple;
  }
...
INT MIdentifySyncPulseRedux(EVENT_HEADER *pheader, void *pevent) {
  ...
  for (int icrate = 0; icrate < NCRATE; ++icrate) {
    ...
    // add to the ntuple either way
    const float vals [3] = {midas_event_number, gData->fTDCSynchronizationPulseIndex.back(), 
gData->fTDCSynchronizationPulseOffset.back()};
    ntupIdentifySyncPulse[icrate]->Fill(vals);
  }
