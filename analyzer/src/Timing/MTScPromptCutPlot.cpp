...
/*-- Module declaration --------------------------------------------*/
static INT MTScPromptCutPlot(EVENT_HEADER*, void*);
...
static TH1* vvhTScPromptCutPlots[MAXNCRATE][MAXNCHAN] = {{}};


ANA_MODULE MTScPromptCutPlot_module =
{
  "MTScPromptCutPlot",    /* module name           */
  "John R Quirk",          /* author                */
  MTScPromptCutPlot,      /* event routine         */
  NULL,                    /* BOR routine           */
  NULL,                    /* EOR routine           */
  NULL, /* init routine          */
  NULL,                    /* exit routine          */
...
INT MTScPromptCutPlot(EVENT_HEADER *pheader, void *pevent) {
  ...
  for (int icrate = 0; icrate < MAXNCRATE; ++icrate) {
    for (int ichan = 0; ichan < MAXNCHAN; ++ichan) {
      ...
      if (pulses_map.count(filtered_bank)) {
        if (!vvhTScPromptCutPlots[icrate][ichan])
          vvhTScPromptCutPlots[icrate][ichan] = histogram_init(bank);
        TH1* hist = vvhTScPromptCutPlots[icrate][ichan];
...
TH1* histogram_init(const char bank[]) {
  ...
  sprintf(name, "hTScPromptCutPlots_%s", bank);
