  ...
  for(int i = 0; i < max_boards; i++) {
    ...
    for(int chan = 0; chan < 8; chan++) {
      ...
      if(enabled) {
        ...
        if(chan % 2 == 0) { 
          setReg("eth1", i, frontend, 0, r0);
          setReg("eth1", i, frontend, 1, r1);
          setReg("eth1", i, frontend, 4, r2);
        } else {
          setReg("eth1", i, frontend, 2, r0);
          setReg("eth1", i, frontend, 3, r1);
          setReg("eth1", i, frontend, 5, r2);
        }
      }
    }
  }
    ...
    for(int i = 0; i < max_boards; i++) {
      ...
      for(int j = 0; j < 4; j++) {
        ...
        for(int k = 0; k < num_packets; k++) {
          ...
          if(pkt) {
            int frontend = (pkt->buffer_number - 1) / 3;
            if(frontend == j) { 
              int len = ntohs(pkt->content_length);
              char *p = ((char *) pkt) + 20;
              memcpy(pdata, p, len);
              if((len % 10) != 0) printf("board %d: content_length = %d !\n", i, len);
              pdata += len;
            }
          }
        }
